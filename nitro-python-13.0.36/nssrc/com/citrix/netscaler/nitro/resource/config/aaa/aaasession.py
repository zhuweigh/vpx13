...
class aaasession(base_resource) :
	...
	@property
	def sessionkey(self) :
		r"""Show aaa session associated with given session key.<br/>Minimum length =  1<br/>Maximum length =  127.
		"""
		try :
	...
	@property
	def port(self) :
		r"""Citrix ADC's port.<br/>Range 1 - 65535<br/>* in CLI is represented as 65535 in NITRO API.
		"""
		try :
			return self._port
		except Exception as e:
	...
	def kill(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				killresource.sessionkey = resource.sessionkey
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						killresources[i].sessionkey = resource[i].sessionkey
