...
class appfwprofile_fieldconsistency_binding(base_resource) :
	...
	@property
	def isregex_ffc(self) :
		r"""Is the web form field name a regular expression?.<br/>Possible values = REGEX, NOTREGEX.
		"""
		try :
			return self._isregex_ffc
		except Exception as e:
			raise e

	@isregex_ffc.setter
	def isregex_ffc(self, isregex_ffc) :
		r"""Is the web form field name a regular expression?.<br/>Possible values = REGEX, NOTREGEX
		"""
		try :
			self._isregex_ffc = isregex_ffc
		except Exception as e:
			raise e
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(appfwprofile_fieldconsistency_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = appfwprofile_fieldconsistency_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [appfwprofile_fieldconsistency_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
						updateresources[i].name = resource[i].name
						updateresources[i].fieldconsistency = resource[i].fieldconsistency
						updateresources[i].formactionurl_ffc = resource[i].formactionurl_ffc
						updateresources[i].isregex_ffc = resource[i].isregex_ffc
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = appfwprofile_fieldconsistency_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [appfwprofile_fieldconsistency_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch appfwprofile_fieldconsistency_binding resources.
		"""
		try :
			if not name :
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of appfwprofile_fieldconsistency_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count appfwprofile_fieldconsistency_binding resources configued on NetScaler.
		"""
		try :
			obj = appfwprofile_fieldconsistency_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of appfwprofile_fieldconsistency_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appfwprofile_fieldconsistency_binding()
...
class appfwprofile_fieldconsistency_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.appfwprofile_fieldconsistency_binding = [appfwprofile_fieldconsistency_binding() for _ in range(length)]
