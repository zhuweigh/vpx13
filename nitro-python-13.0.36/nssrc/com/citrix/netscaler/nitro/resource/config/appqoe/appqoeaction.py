...
class appqoeaction(base_resource) :
	...
	def __init__(self) :
		self._name = None
		self._priority = None
		self._respondwith = None
		self._customfile = None
		self._altcontentsvcname = None
		self._altcontentpath = None
		self._polqdepth = None
		self._priqdepth = None
		self._maxconn = None
		self._delay = None
		self._dostrigexpression = None
		self._dosaction = None
		self._tcpprofile = None
		self._hits = None
		self.___count = None

	@property
	def name(self) :
		r"""Name for the AppQoE action. Must begin with a letter, number, or the underscore symbol (_). Other characters allowed, after the first character, are the hyphen (-), period (.) hash (#), space ( ), at (@), equals (=), and colon (:) characters. This is a mandatory argument.
		"""
		try :
			return self._name
	...
	def respondwith(self) :
		r"""Responder action to be taken when the threshold is reached. Available settings function as follows:
	...
	@property
	def customfile(self) :
		r"""name of the HTML page object to use as the response.<br/>Minimum length =  1.
		"""
		try :
	...
	def altcontentsvcname(self) :
		r"""Name of the alternative content service to be used in the ACS.<br/>Minimum length =  1<br/>Maximum length =  127.
		"""
		try :
	...
	def altcontentpath(self) :
		r"""Path to the alternative content service to be used in the ACS.<br/>Minimum length =  4<br/>Maximum length =  127.
		"""
		try :
	...
	@property
	def polqdepth(self) :
		r"""Policy queue depth threshold value. When the policy queue size (number of requests queued for the policy binding this action is attached to) increases to the specified polqDepth value, subsequent requests are dropped to the lowest priority level.<br/>Maximum length =  0xFFFFFFFE.
		"""
		try :
	...
	@property
	def priqdepth(self) :
		r"""Queue depth threshold value per priorirty level. If the queue size (number of requests in the queue of that particular priorirty) on the virtual server to which this policy is bound, increases to the specified qDepth value, subsequent requests are dropped to the lowest priority level.<br/>Maximum length =  0xFFFFFFFE.
		"""
		try :
	...
	@property
	def maxconn(self) :
		r"""Maximum number of concurrent connections that can be open for requests that matches with rule.<br/>Minimum length =  1<br/>Maximum length =  0xFFFFFFFE.
		"""
		try :
	...
	def dostrigexpression(self) :
		r"""Optional expression to add second level check to trigger DoS actions. Specifically used for Analytics based DoS response generation.
		"""
		try :
	...
	@property
	def dosaction(self) :
		r"""DoS Action to take when vserver will be considered under DoS attack and corresponding rule matches. Mandatory if AppQoE actions are to be used for DoS attack prevention.<br/>Possible values = SimpleResponse, HICResponse.
		"""
		try :
	...
	@property
	def tcpprofile(self) :
		r"""Bind TCP Profile based on L2/L3/L7 parameters.<br/>Minimum length =  1<br/>Maximum length =  127.
		"""
		try :
			return self._tcpprofile
		except Exception as e:
			raise e

	@tcpprofile.setter
	def tcpprofile(self, tcpprofile) :
		r"""Bind TCP Profile based on L2/L3/L7 parameters.<br/>Minimum length =  1<br/>Maximum length =  127
		"""
		try :
			self._tcpprofile = tcpprofile
		except Exception as e:
			raise e
	...
	def add(cls, client, resource) :
		r""" Use this API to add appqoeaction.
		"""
		try :
			if type(resource) is not list :
				...
				addresource.tcpprofile = resource.tcpprofile
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].tcpprofile = resource[i].tcpprofile
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete appqoeaction.
		"""
		try :
			if type(resource) is not list :
	...
	def update(cls, client, resource) :
		r""" Use this API to update appqoeaction.
		"""
		try :
			if type(resource) is not list :
				...
				updateresource.tcpprofile = resource.tcpprofile
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].tcpprofile = resource[i].tcpprofile
	...
	def unset(cls, client, resource, args) :
		r""" Use this API to unset the properties of appqoeaction resource.
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the appqoeaction resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = appqoeaction()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of appqoeaction resources.
	...
	def count(cls, client) :
		r""" Use this API to count the appqoeaction resources configured on NetScaler.
		"""
		try :
			obj = appqoeaction()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of appqoeaction resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appqoeaction()
