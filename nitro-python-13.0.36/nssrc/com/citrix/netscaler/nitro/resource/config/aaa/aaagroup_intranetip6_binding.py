...
class aaagroup_intranetip6_binding(base_resource) :
	""" Binding class showing the intranetip6 that can be bound to aaagroup.
	"""
	def __init__(self) :
	...
	def intranetip6(self) :
		r"""The Intranet IP6(s) bound to the group.
		"""
		try :
	...
	@property
	def numaddr(self) :
		r"""Numbers of ipv6 address bound starting with intranetip6.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(aaagroup_intranetip6_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.aaagroup_intranetip6_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = aaagroup_intranetip6_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [aaagroup_intranetip6_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = aaagroup_intranetip6_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [aaagroup_intranetip6_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, groupname="", option_="") :
		r""" Use this API to fetch aaagroup_intranetip6_binding resources.
		"""
		try :
			if not groupname :
				obj = aaagroup_intranetip6_binding()
	...
	def get_filtered(cls, service, groupname, filter_) :
		r""" Use this API to fetch filtered set of aaagroup_intranetip6_binding resources.
	...
	def count(cls, service, groupname) :
		r""" Use this API to count aaagroup_intranetip6_binding resources configued on NetScaler.
		"""
		try :
			obj = aaagroup_intranetip6_binding()
			obj.groupname = groupname
	...
	def count_filtered(cls, service, groupname, filter_) :
		r""" Use this API to count the filtered set of aaagroup_intranetip6_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaagroup_intranetip6_binding()
...
class aaagroup_intranetip6_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.aaagroup_intranetip6_binding = [aaagroup_intranetip6_binding() for _ in range(length)]
