...
class uservserver(base_resource) :
	...
	@property
	def state(self) :
		r"""Initial state of the user vserver.<br/>Default value: ENABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
			return self._state
		except Exception as e:
	...
	def enable(cls, client, resource) :
		r""" Use this API to enable uservserver.
		"""
		try :
			if type(resource) is not list :
				enableresource = uservserver()
				...
				if type(resource[0]) != cls :
					if (resource and len(resource) > 0) :
						enableresources = [ uservserver() for _ in range(len(resource))]
						for i in range(len(resource)) :
	...
	def disable(cls, client, resource) :
		r""" Use this API to disable uservserver.
		"""
		try :
			if type(resource) is not list :
				disableresource = uservserver()
				...
				if type(resource[0]) != cls :
					if (resource and len(resource) > 0) :
						disableresources = [ uservserver() for _ in range(len(resource))]
						for i in range(len(resource)) :
