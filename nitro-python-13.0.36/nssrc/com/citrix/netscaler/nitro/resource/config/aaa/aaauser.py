...
class aaauser(base_resource) :
	...
	@property
	def username(self) :
		r"""Name for the user. Must begin with a letter, number, or the underscore character (_), and must contain only letters, numbers, and the hyphen (-), period (.) pound (#), space ( ), at (@), equals (=), colon (:), and underscore characters. Cannot be changed after the user is added.
	...
	@property
	def password(self) :
		r"""Password with which the user logs on. Required for any user account that does not exist on an external authentication server. 
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete aaauser.
		"""
		try :
			if type(resource) is not list :
	...
	def update(cls, client, resource) :
		r""" Use this API to update aaauser.
		"""
		try :
			if type(resource) is not list :
	...
	def unlock(cls, client, resource) :
		r""" Use this API to unlock aaauser.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the aaauser resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = aaauser()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of aaauser resources.
	...
	def count(cls, client) :
		r""" Use this API to count the aaauser resources configured on NetScaler.
		"""
		try :
			obj = aaauser()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of aaauser resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaauser()
