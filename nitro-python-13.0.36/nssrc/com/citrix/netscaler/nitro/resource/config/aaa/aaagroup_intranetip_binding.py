...
class aaagroup_intranetip_binding(base_resource) :
	...
	@property
	def intranetip(self) :
		r"""The Intranet IP(s) bound to the group.
		"""
		try :
	...
	@property
	def netmask(self) :
		r"""The netmask for the Intranet IP.
		"""
		try :
	...
	def get(cls, service, groupname="", option_="") :
		r""" Use this API to fetch aaagroup_intranetip_binding resources.
		"""
		try :
			if not groupname :
	...
	def get_filtered(cls, service, groupname, filter_) :
		r""" Use this API to fetch filtered set of aaagroup_intranetip_binding resources.
	...
	def count(cls, service, groupname) :
		r""" Use this API to count aaagroup_intranetip_binding resources configued on NetScaler.
		"""
		try :
			obj = aaagroup_intranetip_binding()
			obj.groupname = groupname
	...
	def count_filtered(cls, service, groupname, filter_) :
		r""" Use this API to count the filtered set of aaagroup_intranetip_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaagroup_intranetip_binding()
