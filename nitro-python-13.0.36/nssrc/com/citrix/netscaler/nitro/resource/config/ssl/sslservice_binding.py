...
class sslservice_binding(base_resource):
	...
	def servicename(self) :
		r"""Name of the SSL service for which to show detailed information.<br/>Minimum length =  1.
		"""
		try :
	...
	def sslservice_sslcipher_bindings(self) :
		r"""sslcipher that can be bound to sslservice.
		"""
		try :
	...
	def sslservice_ecccurve_bindings(self) :
		r"""ecccurve that can be bound to sslservice.
		"""
		try :
	...
	def sslservice_sslciphersuite_bindings(self) :
		r"""sslciphersuite that can be bound to sslservice.
		"""
		try :
	...
	def sslservice_sslpolicy_bindings(self) :
		r"""sslpolicy that can be bound to sslservice.
		"""
		try :
	...
	def sslservice_sslcertkey_bindings(self) :
		r"""sslcertkey that can be bound to sslservice.
		"""
		try :
	...
	@classmethod
	def get(self, service, servicename="", option_="") :
		r""" Use this API to fetch sslservice_binding resource.
		"""
		try :
			if not servicename :
				obj = sslservice_binding()
				response = obj.get_resources(service, option_)
			elif type(servicename) is not list :
				obj = sslservice_binding()
