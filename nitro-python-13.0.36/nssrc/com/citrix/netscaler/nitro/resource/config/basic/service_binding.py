...
class service_binding(base_resource):
	...
	@property
	def name(self) :
		r"""Name of the service for which to display configuration details.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	def service_scpolicy_bindings(self) :
		r"""scpolicy that can be bound to service.
		"""
		try :
	...
	def service_lbmonitor_bindings(self) :
		r"""lbmonitor that can be bound to service.
		"""
		try :
	...
	def service_dospolicy_bindings(self) :
		r"""dospolicy that can be bound to service.
		"""
		try :
	...
	def get(self, service, name="", option_="") :
		r""" Use this API to fetch service_binding resource.
		"""
		try :
			if not name :
				obj = service_binding()
