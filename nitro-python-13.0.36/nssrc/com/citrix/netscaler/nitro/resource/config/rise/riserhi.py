...
class riserhi(base_resource) :
	...
	@property
	def hostrtgw(self) :
		r"""Gateway for the advertised IP.
		"""
		try :
	...
	def nexthopvlan(self) :
		r"""Vlan id on which the gateway is present.
		"""
		try :
	...
	def vserverrhilevel(self) :
		r"""Advertisement level.<br/>Default value: ONE_VSERVER<br/>Possible values = ONE_VSERVER, ALL_VSERVERS, NONE, VSVR_CNTRLD.
		"""
		try :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the riserhi resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = riserhi()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of riserhi resources.
	...
	def count(cls, client) :
		r""" Use this API to count the riserhi resources configured on NetScaler.
		"""
		try :
			obj = riserhi()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of riserhi resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = riserhi()
