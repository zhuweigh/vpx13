...
class sslwrapkey(base_resource) :
	...
	def wrapkeyname(self) :
		r"""Name for the wrap key. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters. Cannot be changed after the wrap key is created.
	...
	@property
	def password(self) :
		r"""Password string for the wrap key.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def salt(self) :
		r"""Salt string for the wrap key.<br/>Minimum length =  1.
		"""
		try :
			return self._salt
		except Exception as e:
	...
	def create(cls, client, resource) :
		r""" Use this API to create sslwrapkey.
		"""
		try :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete sslwrapkey.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the sslwrapkey resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = sslwrapkey()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of sslwrapkey resources.
	...
	def count(cls, client) :
		r""" Use this API to count the sslwrapkey resources configured on NetScaler.
		"""
		try :
			obj = sslwrapkey()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of sslwrapkey resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = sslwrapkey()
