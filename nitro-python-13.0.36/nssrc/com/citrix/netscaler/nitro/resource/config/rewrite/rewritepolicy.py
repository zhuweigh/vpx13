...
class rewritepolicy(base_resource) :
	...
	@property
	def name(self) :
		r"""Name for the rewrite policy. Must begin with a letter, number, or the underscore character (_), and must contain only letters, numbers, and the hyphen (-), period (.) hash (#), space ( ), at (@), equals (=), colon (:), and underscore characters. Can be changed after the rewrite policy is added.
	...
	@property
	def action(self) :
		r"""Name of the rewrite action to perform if the request or response matches this rewrite policy.
	...
	@property
	def comment(self) :
		r"""Any comments to preserve information about this rewrite policy.
		"""
		try :
	...
	@property
	def newname(self) :
		r"""New name for the rewrite policy. 
	...
	@property
	def isdefault(self) :
		r"""A value of true is returned if it is a default rewritepolicy.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(rewritepolicy_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		r""" Use this API to add rewritepolicy.
		"""
		try :
			if type(resource) is not list :
				addresource = rewritepolicy()
				...
				if (resource and len(resource) > 0) :
					addresources = [ rewritepolicy() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete rewritepolicy.
		"""
		try :
			if type(resource) is not list :
				deleteresource = rewritepolicy()
				...
				if type(resource[0]) != cls :
					if (resource and len(resource) > 0) :
						deleteresources = [ rewritepolicy() for _ in range(len(resource))]
						for i in range(len(resource)) :
	...
	def update(cls, client, resource) :
		r""" Use this API to update rewritepolicy.
		"""
		try :
			if type(resource) is not list :
				updateresource = rewritepolicy()
				...
				if (resource and len(resource) > 0) :
					updateresources = [ rewritepolicy() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def unset(cls, client, resource, args) :
		r""" Use this API to unset the properties of rewritepolicy resource.
		Properties that need to be unset are specified in args array.
		"""
		try :
			if type(resource) is not list :
				unsetresource = rewritepolicy()
				...
				if type(resource[0]) != cls :
					if (resource and len(resource) > 0) :
						unsetresources = [ rewritepolicy() for _ in range(len(resource))]
						for i in range(len(resource)) :
	...
	def rename(cls, client, resource, new_name) :
		r""" Use this API to rename a rewritepolicy resource.
		"""
		try :
			renameresource = rewritepolicy()
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the rewritepolicy resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = rewritepolicy()
				...
				if type(name) is not list :
					...
					if name and len(name) > 0 :
						...
						response = [rewritepolicy() for _ in range(len(name))]
						obj = [rewritepolicy() for _ in range(len(name))]
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of rewritepolicy resources.
	...
	def count(cls, client) :
		r""" Use this API to count the rewritepolicy resources configured on NetScaler.
		"""
		try :
			obj = rewritepolicy()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of rewritepolicy resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = rewritepolicy()
...
class rewritepolicy_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.rewritepolicy = [rewritepolicy() for _ in range(length)]
