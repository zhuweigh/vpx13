...
class contentinspectionpolicylabel_policybinding_binding(base_resource) :
	""" Binding class showing the policybinding that can be bound to contentinspectionpolicylabel.
	"""
	def __init__(self) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(contentinspectionpolicylabel_policybinding_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.contentinspectionpolicylabel_policybinding_binding
		except Exception as e :
			raise e
	...
	def get(cls, service, labelname="", option_="") :
		r""" Use this API to fetch contentinspectionpolicylabel_policybinding_binding resources.
		"""
		try :
			if not labelname :
				obj = contentinspectionpolicylabel_policybinding_binding()
	...
	def get_filtered(cls, service, labelname, filter_) :
		r""" Use this API to fetch filtered set of contentinspectionpolicylabel_policybinding_binding resources.
	...
	def count(cls, service, labelname) :
		r""" Use this API to count contentinspectionpolicylabel_policybinding_binding resources configued on NetScaler.
		"""
		try :
			obj = contentinspectionpolicylabel_policybinding_binding()
			obj.labelname = labelname
	...
	def count_filtered(cls, service, labelname, filter_) :
		r""" Use this API to count the filtered set of contentinspectionpolicylabel_policybinding_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = contentinspectionpolicylabel_policybinding_binding()
...
class contentinspectionpolicylabel_policybinding_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.contentinspectionpolicylabel_policybinding_binding = [contentinspectionpolicylabel_policybinding_binding() for _ in range(length)]
