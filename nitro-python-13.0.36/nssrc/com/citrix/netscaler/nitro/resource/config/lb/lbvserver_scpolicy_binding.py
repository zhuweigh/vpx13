...
class lbvserver_scpolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(lbvserver_scpolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = lbvserver_scpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [lbvserver_scpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = lbvserver_scpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [lbvserver_scpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch lbvserver_scpolicy_binding resources.
		"""
		try :
			if not name :
				obj = lbvserver_scpolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of lbvserver_scpolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count lbvserver_scpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = lbvserver_scpolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of lbvserver_scpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = lbvserver_scpolicy_binding()
...
class lbvserver_scpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.lbvserver_scpolicy_binding = [lbvserver_scpolicy_binding() for _ in range(length)]
