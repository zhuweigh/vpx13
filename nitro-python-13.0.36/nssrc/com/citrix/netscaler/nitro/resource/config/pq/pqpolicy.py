...
class pqpolicy(base_resource) :
	...
	@property
	def policyname(self) :
		r"""Name for the priority queuing policy. Must begin with a letter, number, or the underscore symbol (_). Other characters allowed, after the first character, are the hyphen (-), period (.) hash (#), space ( ), at (@), equals (=), and colon (:) characters.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def rule(self) :
		r"""Expression or name of a named expression, against which the request is evaluated. The priority queuing policy is applied if the rule evaluates to true.
		Note:
	...
	@property
	def weight(self) :
		r"""Weight of the priority. Each priority is assigned a weight according to which it is served when server resources are available. The weight for a higher priority request must be set higher than that of a lower priority request.
	...
	@property
	def qdepth(self) :
		r"""Queue depth threshold value. When the queue size (number of requests in the queue) on the virtual server to which this policy is bound, increases to the specified qDepth value, subsequent requests are dropped to the lowest priority level.<br/>Default value: 0<br/>Maximum length =  0xFFFFFFFE.
		"""
		try :
			return self._qdepth
		except Exception as e:
	...
	@property
	def polqdepth(self) :
		r"""Policy queue depth threshold value. When the policy queue size (number of requests in all the queues belonging to this policy) increases to the specified polqDepth value, subsequent requests are dropped to the lowest priority level.<br/>Default value: 0<br/>Maximum length =  0xFFFFFFFE.
		"""
		try :
	...
	def add(cls, client, resource) :
		r""" Use this API to add pqpolicy.
		"""
		try :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete pqpolicy.
		"""
		try :
			if type(resource) is not list :
	...
	def update(cls, client, resource) :
		r""" Use this API to update pqpolicy.
		"""
		try :
			if type(resource) is not list :
	...
	def unset(cls, client, resource, args) :
		r""" Use this API to unset the properties of pqpolicy resource.
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the pqpolicy resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = pqpolicy()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of pqpolicy resources.
	...
	def count(cls, client) :
		r""" Use this API to count the pqpolicy resources configured on NetScaler.
		"""
		try :
			obj = pqpolicy()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of pqpolicy resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = pqpolicy()
