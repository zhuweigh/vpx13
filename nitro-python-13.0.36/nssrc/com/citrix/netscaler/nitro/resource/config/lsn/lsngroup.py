...
class lsngroup(base_resource) :
	...
	def allocpolicy(self) :
		...
		2.2.2.1 => 4.4.4.1:PB1
		2.2.2.2 => 4.4.4.2:PB1
		2.2.2.3 => 4.4.4.1:PB2.<br/>Default value: IPADDRS<br/>Possible values = PORTS, IPADDRS.
		"""
		try :
	...
	@property
	def logging(self) :
		r"""Log mapping entries and sessions created or deleted for this LSN group. The Citrix ADC logs LSN sessions for this LSN group only when both logging and session logging parameters are enabled.
	...
	def sessionlogging(self) :
		r"""Log sessions created or deleted for the LSN group. The Citrix ADC logs LSN sessions for this LSN group only when both logging and session logging parameters are enabled.
	...
	@property
	def ftpcm(self) :
		r"""Enable the FTP connection mirroring for specified LSN group. Connection mirroring (CM or connection failover) refers to keeping active an established TCP or UDP connection when a failover occurs.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
			return self._ftpcm
		except Exception as e:
	...
	def add(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				addresource.ftpcm = resource.ftpcm
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].ftpcm = resource[i].ftpcm
	...
	def update(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				updateresource.ftpcm = resource.ftpcm
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].ftpcm = resource[i].ftpcm
