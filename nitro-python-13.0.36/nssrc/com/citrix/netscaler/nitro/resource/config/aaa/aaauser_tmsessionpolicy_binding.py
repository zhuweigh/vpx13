...
class aaauser_tmsessionpolicy_binding(base_resource) :
	""" Binding class showing the tmsessionpolicy that can be bound to aaauser.
	"""
	def __init__(self) :
		self._policy = None
		self._priority = None
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(aaauser_tmsessionpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.aaauser_tmsessionpolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = aaauser_tmsessionpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [aaauser_tmsessionpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = aaauser_tmsessionpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [aaauser_tmsessionpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, username="", option_="") :
		r""" Use this API to fetch aaauser_tmsessionpolicy_binding resources.
		"""
		try :
			if not username :
				obj = aaauser_tmsessionpolicy_binding()
	...
	def get_filtered(cls, service, username, filter_) :
		r""" Use this API to fetch filtered set of aaauser_tmsessionpolicy_binding resources.
	...
	def count(cls, service, username) :
		r""" Use this API to count aaauser_tmsessionpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = aaauser_tmsessionpolicy_binding()
			obj.username = username
	...
	def count_filtered(cls, service, username, filter_) :
		r""" Use this API to count the filtered set of aaauser_tmsessionpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaauser_tmsessionpolicy_binding()
...
class aaauser_tmsessionpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.aaauser_tmsessionpolicy_binding = [aaauser_tmsessionpolicy_binding() for _ in range(length)]
