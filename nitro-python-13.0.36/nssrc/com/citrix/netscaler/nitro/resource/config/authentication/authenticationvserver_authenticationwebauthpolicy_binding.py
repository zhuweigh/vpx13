...
class authenticationvserver_authenticationwebauthpolicy_binding(base_resource) :
	""" Binding class showing the authenticationwebauthpolicy that can be bound to authenticationvserver.
	"""
	def __init__(self) :
		self._policy = None
		self._priority = None
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(authenticationvserver_authenticationwebauthpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.authenticationvserver_authenticationwebauthpolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = authenticationvserver_authenticationwebauthpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [authenticationvserver_authenticationwebauthpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = authenticationvserver_authenticationwebauthpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [authenticationvserver_authenticationwebauthpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch authenticationvserver_authenticationwebauthpolicy_binding resources.
		"""
		try :
			if not name :
				obj = authenticationvserver_authenticationwebauthpolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of authenticationvserver_authenticationwebauthpolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count authenticationvserver_authenticationwebauthpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = authenticationvserver_authenticationwebauthpolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of authenticationvserver_authenticationwebauthpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = authenticationvserver_authenticationwebauthpolicy_binding()
...
class authenticationvserver_authenticationwebauthpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.authenticationvserver_authenticationwebauthpolicy_binding = [authenticationvserver_authenticationwebauthpolicy_binding() for _ in range(length)]
