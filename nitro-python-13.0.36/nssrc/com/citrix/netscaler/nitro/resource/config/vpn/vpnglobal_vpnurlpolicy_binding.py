...
class vpnglobal_vpnurlpolicy_binding(base_resource) :
	""" Binding class showing the vpnurlpolicy that can be bound to vpnglobal.
	"""
	def __init__(self) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(vpnglobal_vpnurlpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.vpnglobal_vpnurlpolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = vpnglobal_vpnurlpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [vpnglobal_vpnurlpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = vpnglobal_vpnurlpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [vpnglobal_vpnurlpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	@classmethod
	def get(cls, service) :
		r""" Use this API to fetch a vpnglobal_vpnurlpolicy_binding resources.
		"""
		try :
			obj = vpnglobal_vpnurlpolicy_binding()
	...
	def get_filtered(cls, service, filter_) :
		r""" Use this API to fetch filtered set of vpnglobal_vpnurlpolicy_binding resources.
	...
	def count(cls, service) :
		r""" Use this API to count vpnglobal_vpnurlpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = vpnglobal_vpnurlpolicy_binding()
			option_ = options()
	...
	def count_filtered(cls, service, filter_) :
		r""" Use this API to count the filtered set of vpnglobal_vpnurlpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vpnglobal_vpnurlpolicy_binding()
...
class vpnglobal_vpnurlpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		self.vpnglobal_vpnurlpolicy_binding = []
		self.errorcode = 0
		self.message = ""
		self.severity = ""
		self.sessionid = ""
		self.vpnglobal_vpnurlpolicy_binding = [vpnglobal_vpnurlpolicy_binding() for _ in range(length)]
