...
class aaagroup_auditnslogpolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(aaagroup_auditnslogpolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = aaagroup_auditnslogpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [aaagroup_auditnslogpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = aaagroup_auditnslogpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [aaagroup_auditnslogpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, groupname="", option_="") :
		r""" Use this API to fetch aaagroup_auditnslogpolicy_binding resources.
		"""
		try :
			if not groupname :
				obj = aaagroup_auditnslogpolicy_binding()
	...
	def get_filtered(cls, service, groupname, filter_) :
		r""" Use this API to fetch filtered set of aaagroup_auditnslogpolicy_binding resources.
	...
	def count(cls, service, groupname) :
		r""" Use this API to count aaagroup_auditnslogpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = aaagroup_auditnslogpolicy_binding()
			obj.groupname = groupname
	...
	def count_filtered(cls, service, groupname, filter_) :
		r""" Use this API to count the filtered set of aaagroup_auditnslogpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaagroup_auditnslogpolicy_binding()
...
class aaagroup_auditnslogpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.aaagroup_auditnslogpolicy_binding = [aaagroup_auditnslogpolicy_binding() for _ in range(length)]
