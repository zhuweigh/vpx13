...
class appfwpolicylabel_appfwpolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(appfwpolicylabel_appfwpolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = appfwpolicylabel_appfwpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [appfwpolicylabel_appfwpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = appfwpolicylabel_appfwpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [appfwpolicylabel_appfwpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, labelname="", option_="") :
		r""" Use this API to fetch appfwpolicylabel_appfwpolicy_binding resources.
		"""
		try :
			if not labelname :
				obj = appfwpolicylabel_appfwpolicy_binding()
	...
	def get_filtered(cls, service, labelname, filter_) :
		r""" Use this API to fetch filtered set of appfwpolicylabel_appfwpolicy_binding resources.
	...
	def count(cls, service, labelname) :
		r""" Use this API to count appfwpolicylabel_appfwpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = appfwpolicylabel_appfwpolicy_binding()
			obj.labelname = labelname
	...
	def count_filtered(cls, service, labelname, filter_) :
		r""" Use this API to count the filtered set of appfwpolicylabel_appfwpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appfwpolicylabel_appfwpolicy_binding()
...
class appfwpolicylabel_appfwpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.appfwpolicylabel_appfwpolicy_binding = [appfwpolicylabel_appfwpolicy_binding() for _ in range(length)]
