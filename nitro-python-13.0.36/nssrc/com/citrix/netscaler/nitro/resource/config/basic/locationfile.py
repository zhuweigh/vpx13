...
class locationfile(base_resource) :
	...
	def Locationfile(self) :
		r"""Name of the location file, with or without absolute path. If the path is not included, the default path (/var/netscaler/locdb) is assumed. In a high availability setup, the static database must be stored in the same location on both Citrix ADCs.<br/>Minimum length =  1.
		"""
		try :
	...
	def add(cls, client, resource) :
		r""" Use this API to add locationfile.
		"""
		try :
	...
	def delete(cls, client, resource="") :
		r""" Use this API to delete locationfile.
		"""
		try :
			if type(resource) is not list :
	...
	def Import(cls, client, resource) :
		r""" Use this API to Import locationfile.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the locationfile resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = locationfile()
