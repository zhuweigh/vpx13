...
class vlan_channel_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(vlan_channel_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = vlan_channel_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [vlan_channel_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = vlan_channel_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [vlan_channel_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, id="", option_="") :
		r""" Use this API to fetch vlan_channel_binding resources.
		"""
		try :
			if not id :
				obj = vlan_channel_binding()
	...
	def get_filtered(cls, service, id, filter_) :
		r""" Use this API to fetch filtered set of vlan_channel_binding resources.
	...
	def count(cls, service, id) :
		r""" Use this API to count vlan_channel_binding resources configued on NetScaler.
		"""
		try :
			obj = vlan_channel_binding()
			obj.id = id
	...
	def count_filtered(cls, service, id, filter_) :
		r""" Use this API to count the filtered set of vlan_channel_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vlan_channel_binding()
...
class vlan_channel_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.vlan_channel_binding = [vlan_channel_binding() for _ in range(length)]
