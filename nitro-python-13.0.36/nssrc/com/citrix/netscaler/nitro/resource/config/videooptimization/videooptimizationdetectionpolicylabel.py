...
class videooptimizationdetectionpolicylabel(base_resource) :
	...
	@property
	def labelname(self) :
		r"""Name for the Video optimization detection policy label. Must begin with a letter, number, or the underscore character (_), and must contain only letters, numbers, and the hyphen (-), period (
	...
	@property
	def comment(self) :
		r"""Any comments to preserve information about this videooptimization detection policy label.
		"""
		try :
	...
	@property
	def newname(self) :
		r"""New name for the videooptimization detection policy label. Must begin with a letter, number, or the underscore character (_), and must contain only letters, numbers, and the hyphen (
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(videooptimizationdetectionpolicylabel_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.videooptimizationdetectionpolicylabel
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		r""" Use this API to add videooptimizationdetectionpolicylabel.
		"""
		try :
			if type(resource) is not list :
				addresource = videooptimizationdetectionpolicylabel()
				...
				if (resource and len(resource) > 0) :
					addresources = [ videooptimizationdetectionpolicylabel() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete videooptimizationdetectionpolicylabel.
		"""
		try :
			if type(resource) is not list :
				deleteresource = videooptimizationdetectionpolicylabel()
				...
				if type(resource[0]) != cls :
					if (resource and len(resource) > 0) :
						deleteresources = [ videooptimizationdetectionpolicylabel() for _ in range(len(resource))]
						for i in range(len(resource)) :
	...
	def rename(cls, client, resource, new_labelname) :
		r""" Use this API to rename a videooptimizationdetectionpolicylabel resource.
		"""
		try :
			renameresource = videooptimizationdetectionpolicylabel()
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the videooptimizationdetectionpolicylabel resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = videooptimizationdetectionpolicylabel()
				...
				if type(name) is not list :
					...
					if name and len(name) > 0 :
						...
						response = [videooptimizationdetectionpolicylabel() for _ in range(len(name))]
						obj = [videooptimizationdetectionpolicylabel() for _ in range(len(name))]
						for i in range(len(name)) :
							obj[i] = videooptimizationdetectionpolicylabel()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of videooptimizationdetectionpolicylabel resources.
	...
	def count(cls, client) :
		r""" Use this API to count the videooptimizationdetectionpolicylabel resources configured on NetScaler.
		"""
		try :
			obj = videooptimizationdetectionpolicylabel()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of videooptimizationdetectionpolicylabel resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = videooptimizationdetectionpolicylabel()
...
class videooptimizationdetectionpolicylabel_response(base_response) :
	def __init__(self, length=1) :
		self.videooptimizationdetectionpolicylabel = []
		self.errorcode = 0
		self.message = ""
		self.severity = ""
		self.sessionid = ""
		self.videooptimizationdetectionpolicylabel = [videooptimizationdetectionpolicylabel() for _ in range(length)]
