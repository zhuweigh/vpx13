...
class vlan_linkset_binding(base_resource) :
	...
	def get(cls, service, id="", option_="") :
		r""" Use this API to fetch vlan_linkset_binding resources.
		"""
		try :
			if not id :
	...
	def get_filtered(cls, service, id, filter_) :
		r""" Use this API to fetch filtered set of vlan_linkset_binding resources.
	...
	def count(cls, service, id) :
		r""" Use this API to count vlan_linkset_binding resources configued on NetScaler.
		"""
		try :
			obj = vlan_linkset_binding()
			obj.id = id
	...
	def count_filtered(cls, service, id, filter_) :
		r""" Use this API to count the filtered set of vlan_linkset_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vlan_linkset_binding()
