...
class lsnappsprofile_port_binding(base_resource) :
	...
	@property
	def lsnport(self) :
		r"""Port numbers or range of port numbers to match against the destination port of the incoming packet from a subscriber. When the destination port is matched, the LSN application profile is applied for the LSN session. Separate a range of ports with a hyphen. For example, 40-90.<br/>Minimum length =  1.
		"""
		try :
