...
class appfwprofile_starturl_binding(base_resource) :
	""" Binding class showing the starturl that can be bound to appfwprofile.
	"""
	def __init__(self) :
		self._starturl = None
		self._state = None
	...
	@property
	def starturl(self) :
		r"""A regular expression that designates a URL on the Start URL list.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(appfwprofile_starturl_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.appfwprofile_starturl_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = appfwprofile_starturl_binding()
				updateresource.name = resource.name
				updateresource.starturl = resource.starturl
				updateresource.comment = resource.comment
				updateresource.state = resource.state
				return updateresource.update_resource(client)
			else :
				if resource and len(resource) > 0 :
					updateresources = [appfwprofile_starturl_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
						updateresources[i].name = resource[i].name
						updateresources[i].starturl = resource[i].starturl
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = appfwprofile_starturl_binding()
				deleteresource.name = resource.name
				deleteresource.starturl = resource.starturl
				return deleteresource.delete_resource(client)
			else :
				if resource and len(resource) > 0 :
					deleteresources = [appfwprofile_starturl_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch appfwprofile_starturl_binding resources.
		"""
		try :
			if not name :
				obj = appfwprofile_starturl_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of appfwprofile_starturl_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count appfwprofile_starturl_binding resources configued on NetScaler.
		"""
		try :
			obj = appfwprofile_starturl_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of appfwprofile_starturl_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appfwprofile_starturl_binding()
...
class appfwprofile_starturl_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.appfwprofile_starturl_binding = [appfwprofile_starturl_binding() for _ in range(length)]
