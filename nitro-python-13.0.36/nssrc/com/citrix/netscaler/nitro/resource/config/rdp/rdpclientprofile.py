...
class rdpclientprofile(base_resource) :
	...
	def __init__(self) :
		...
		self._psk = None
		self._randomizerdpfilename = None
		self._rdplinkattribute = None
		self._feature = None
		self.___count = None
	...
	def rdpcookievalidity(self) :
		r"""RDP cookie validity period. RDP cookie validity time is applicable for new connection and also for any re-connection that might happen, mostly due to network disruption or during fail-over.<br/>Default value: 60<br/>Minimum length =  1<br/>Maximum length =  86400.
		"""
		try :
	...
	def rdplistener(self) :
		r"""IP address (or) Fully-qualified domain name(FQDN) of the RDP Listener with the port in the format IP:Port (or) FQDN:Port.<br/>Maximum length =  255.
		"""
		try :
	...
	@property
	def randomizerdpfilename(self) :
		r"""Will generate unique filename everytime rdp file is downloaded by appending output of time() function in the format <rdpfileName>_<time>.rdp. This tries to avoid the pop-up for replacement of existing rdp file during each rdp connection launch, hence providing better end-user experience.<br/>Default value: NO<br/>Possible values = YES, NO.
		"""
		try :
			return self._randomizerdpfilename
		except Exception as e:
			raise e

	@randomizerdpfilename.setter
	def randomizerdpfilename(self, randomizerdpfilename) :
		r"""Will generate unique filename everytime rdp file is downloaded by appending output of time() function in the format <rdpfileName>_<time>.rdp. This tries to avoid the pop-up for replacement of existing rdp file during each rdp connection launch, hence providing better end-user experience.<br/>Default value: NO<br/>Possible values = YES, NO
		"""
		try :
			self._randomizerdpfilename = randomizerdpfilename
		except Exception as e:
			raise e

	@property
	def rdplinkattribute(self) :
		r"""Citrix Gateway allows the configuration of rdpLinkAttribute parameter which can be used to fetch a list of RDP servers(IP/FQDN) that a user can access, from an Authentication server attribute(Example: LDAP, SAML). Based on the list received, the RDP links will be generated and displayed to the user.
		Note: The Attribute mentioned in the rdpLinkAttribute should be fetched through corresponding authentication method.
		"""
		try :
			return self._rdplinkattribute
		except Exception as e:
			raise e

	@rdplinkattribute.setter
	def rdplinkattribute(self, rdplinkattribute) :
		r"""Citrix Gateway allows the configuration of rdpLinkAttribute parameter which can be used to fetch a list of RDP servers(IP/FQDN) that a user can access, from an Authentication server attribute(Example: LDAP, SAML). Based on the list received, the RDP links will be generated and displayed to the user.
		Note: The Attribute mentioned in the rdpLinkAttribute should be fetched through corresponding authentication method.
		"""
		try :
			self._rdplinkattribute = rdplinkattribute
		except Exception as e:
			raise e
	...
	def add(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				addresource.randomizerdpfilename = resource.randomizerdpfilename
				addresource.rdplinkattribute = resource.rdplinkattribute
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].randomizerdpfilename = resource[i].randomizerdpfilename
						addresources[i].rdplinkattribute = resource[i].rdplinkattribute
	...
	def update(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				updateresource.randomizerdpfilename = resource.randomizerdpfilename
				updateresource.rdplinkattribute = resource.rdplinkattribute
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].randomizerdpfilename = resource[i].randomizerdpfilename
						updateresources[i].rdplinkattribute = resource[i].rdplinkattribute
