...
class streamselector(base_resource) :
	...
	@property
	def rule(self) :
		r"""Set of up to five expressions. Maximum length: 7499 characters. Each expression must identify a specific request characteristic, such as the client's IP address (with CLIENT.IP.SRC) or requested server resource (with HTTP.REQ.URL).
	...
	def add(cls, client, resource) :
		r""" Use this API to add streamselector.
		"""
		try :
	...
	def update(cls, client, resource) :
		r""" Use this API to update streamselector.
		"""
		try :
			if type(resource) is not list :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete streamselector.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the streamselector resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = streamselector()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of streamselector resources.
	...
	def count(cls, client) :
		r""" Use this API to count the streamselector resources configured on NetScaler.
		"""
		try :
			obj = streamselector()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of streamselector resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = streamselector()
