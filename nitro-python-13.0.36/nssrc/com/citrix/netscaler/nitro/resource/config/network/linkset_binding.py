...
class linkset_binding(base_resource):
	...
	@property
	def id(self) :
		r"""ID of the linkset for which to display information. If an ID is not provided, the display includes information about all linksets that are available in the cluster.
		"""
		try :
			return self._id
	...
	def linkset_channel_bindings(self) :
		r"""channel that can be bound to linkset.
		"""
		try :
	...
	def linkset_interface_bindings(self) :
		r"""interface that can be bound to linkset.
		"""
		try :
	...
	def get(self, service, id="", option_="") :
		r""" Use this API to fetch linkset_binding resource.
		"""
		try :
			if not id :
				obj = linkset_binding()
