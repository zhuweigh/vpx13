...
class cacheforwardproxy(base_resource) :
	...
	@property
	def ipaddress(self) :
		r"""IP address of the Citrix ADC or a cache server for which the cache acts as a proxy. Requests coming to the Citrix ADC with the configured IP address are forwarded to the particular address, without involving the Integrated Cache in any way.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def port(self) :
		r"""Port on the Citrix ADC or a server for which the cache acts as a proxy.<br/>Minimum length =  1<br/>Range 1 - 65535<br/>* in CLI is represented as 65535 in NITRO API.
		"""
		try :
			return self._port
		except Exception as e:
	...
	def add(cls, client, resource) :
		r""" Use this API to add cacheforwardproxy.
		"""
		try :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete cacheforwardproxy.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the cacheforwardproxy resources that are configured on netscaler.
		"""
		try :
			if not name :
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of cacheforwardproxy resources.
	...
	def count(cls, client) :
		r""" Use this API to count the cacheforwardproxy resources configured on NetScaler.
		"""
		try :
			obj = cacheforwardproxy()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of cacheforwardproxy resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = cacheforwardproxy()
