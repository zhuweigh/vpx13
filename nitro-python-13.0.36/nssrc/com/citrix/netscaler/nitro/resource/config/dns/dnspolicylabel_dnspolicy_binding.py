...
class dnspolicylabel_dnspolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(dnspolicylabel_dnspolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = dnspolicylabel_dnspolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [dnspolicylabel_dnspolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = dnspolicylabel_dnspolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [dnspolicylabel_dnspolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, labelname="", option_="") :
		r""" Use this API to fetch dnspolicylabel_dnspolicy_binding resources.
		"""
		try :
			if not labelname :
				obj = dnspolicylabel_dnspolicy_binding()
	...
	def get_filtered(cls, service, labelname, filter_) :
		r""" Use this API to fetch filtered set of dnspolicylabel_dnspolicy_binding resources.
	...
	def count(cls, service, labelname) :
		r""" Use this API to count dnspolicylabel_dnspolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = dnspolicylabel_dnspolicy_binding()
			obj.labelname = labelname
	...
	def count_filtered(cls, service, labelname, filter_) :
		r""" Use this API to count the filtered set of dnspolicylabel_dnspolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = dnspolicylabel_dnspolicy_binding()
...
class dnspolicylabel_dnspolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.dnspolicylabel_dnspolicy_binding = [dnspolicylabel_dnspolicy_binding() for _ in range(length)]
