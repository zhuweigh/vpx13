...
class vpnvserver_vpnurlpolicy_binding(base_resource) :
	""" Binding class showing the vpnurlpolicy that can be bound to vpnvserver.
	"""
	def __init__(self) :
		self._policy = None
		self._priority = None
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(vpnvserver_vpnurlpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.vpnvserver_vpnurlpolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = vpnvserver_vpnurlpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [vpnvserver_vpnurlpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = vpnvserver_vpnurlpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [vpnvserver_vpnurlpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch vpnvserver_vpnurlpolicy_binding resources.
		"""
		try :
			if not name :
				obj = vpnvserver_vpnurlpolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of vpnvserver_vpnurlpolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count vpnvserver_vpnurlpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = vpnvserver_vpnurlpolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of vpnvserver_vpnurlpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vpnvserver_vpnurlpolicy_binding()
...
class vpnvserver_vpnurlpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		self.vpnvserver_vpnurlpolicy_binding = []
		self.errorcode = 0
		self.message = ""
		self.severity = ""
		self.sessionid = ""
		self.vpnvserver_vpnurlpolicy_binding = [vpnvserver_vpnurlpolicy_binding() for _ in range(length)]
