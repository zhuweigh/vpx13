...
class nsstats(base_resource) :
	...
	def __init__(self) :
		self._cleanuplevel = None

	@property
	def cleanuplevel(self) :
		r"""The level of stats to be cleared. 'global' option will clear global counters only, 'all' option will clear all device counters also along with global counters. For both the cases only 'ever incrementing counters' i.e. total counters will be cleared.<br/>Possible values = global, all.
		"""
		try :
	...
	def clear(cls, client, resource) :
		r""" Use this API to clear nsstats.
		"""
		try :
