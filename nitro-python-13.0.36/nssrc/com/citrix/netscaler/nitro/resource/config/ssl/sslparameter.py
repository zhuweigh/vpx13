...
class sslparameter(base_resource) :
	...
	def softwarecryptothreshold(self) :
		r"""Citrix ADC CPU utilization threshold (in percentage) beyond which crypto operations are not done in software.
	...
	def sigdigesttype(self) :
		r"""Signature Digest Algorithms that are supported by appliance. Default value is "ALL" and it will enable the following algorithms depending on the platform.
		...
		<br/>Default value: ALL<br/>Possible values = ALL, RSA-MD5, RSA-SHA1, RSA-SHA224, RSA-SHA256, RSA-SHA384, RSA-SHA512, DSA-SHA1, DSA-SHA224, DSA-SHA256, DSA-SHA384, DSA-SHA512, ECDSA-SHA1, ECDSA-SHA224, ECDSA-SHA256, ECDSA-SHA384, ECDSA-SHA512.
		"""
		try :
	...
	@property
	def ndcppcompliancecertcheck(self) :
		r"""When netscaler is acting as a client (Backend connection), if this option is set to YES, during the certificate verification netscaler will ignore common name if SAN is present.<br/>Default value: NO<br/>Possible values = YES, NO.
		"""
		try :
			return self._ndcppcompliancecertcheck
		except Exception as e:
			raise e

	@ndcppcompliancecertcheck.setter
	def ndcppcompliancecertcheck(self, ndcppcompliancecertcheck) :
		r"""When netscaler is acting as a client (Backend connection), if this option is set to YES, during the certificate verification netscaler will ignore common name if SAN is present.<br/>Default value: NO<br/>Possible values = YES, NO
		"""
		try :
			self._ndcppcompliancecertcheck = ndcppcompliancecertcheck
		except Exception as e:
			raise e
	...
	def update(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				updateresource.ndcppcompliancecertcheck = resource.ndcppcompliancecertcheck
	...
	class Ndcppcompliancecertcheck:
		YES = "YES"
		NO = "NO"
