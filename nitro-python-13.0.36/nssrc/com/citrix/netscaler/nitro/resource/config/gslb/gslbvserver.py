...
class gslbvserver(base_resource) :
	...
	@property
	def lbmethod(self) :
		r"""Load balancing method for the GSLB virtual server.<br/>Default value: LEASTCONNECTION<br/>Possible values = ROUNDROBIN, LEASTCONNECTION, LEASTRESPONSETIME, SOURCEIPHASH, LEASTBANDWIDTH, LEASTPACKETS, STATICPROXIMITY, RTT, CUSTOMLOAD.
		"""
		try :
			...
			if type(resource) is not list :
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].ecs = resource[i].ecs
						addresources[i].ecsaddrvalidation = resource[i].ecsaddrvalidation
			...
			if type(resource) is not list :
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].ecs = resource[i].ecs
						updateresources[i].ecsaddrvalidation = resource[i].ecsaddrvalidation
