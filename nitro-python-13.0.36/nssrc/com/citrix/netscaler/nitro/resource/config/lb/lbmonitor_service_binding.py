...
class lbmonitor_service_binding(base_resource) :
	...
	@property
	def dup_state(self) :
		r"""State of the monitor. The state setting for a monitor of a given type affects all monitors of that type. For example, if an HTTP monitor is enabled, all HTTP monitors on the appliance are (or remain) enabled. If an HTTP monitor is disabled, all HTTP monitors on the appliance are disabled.
		"""
		try :
	...
	@property
	def dup_weight(self) :
		r"""Weight to assign to the binding between the monitor and service.
		"""
		try :
