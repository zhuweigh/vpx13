...
class nspartition_bridgegroup_binding(base_resource) :
	""" Binding class showing the bridgegroup that can be bound to nspartition.
	"""
	def __init__(self) :
		self._bridgegroup = None
		self._partitionname = None
		self.___count = None

	@property
	def bridgegroup(self) :
		r"""Identifier of the bridge group that is assigned to this partition.<br/>Minimum value =  1<br/>Maximum value =  1000.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(nspartition_bridgegroup_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.nspartition_bridgegroup_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = nspartition_bridgegroup_binding()
				updateresource.partitionname = resource.partitionname
				updateresource.bridgegroup = resource.bridgegroup
				return updateresource.update_resource(client)
			else :
				if resource and len(resource) > 0 :
					updateresources = [nspartition_bridgegroup_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = nspartition_bridgegroup_binding()
				deleteresource.partitionname = resource.partitionname
				deleteresource.bridgegroup = resource.bridgegroup
				return deleteresource.delete_resource(client)
			else :
				if resource and len(resource) > 0 :
					deleteresources = [nspartition_bridgegroup_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, partitionname="", option_="") :
		r""" Use this API to fetch nspartition_bridgegroup_binding resources.
		"""
		try :
			if not partitionname :
				obj = nspartition_bridgegroup_binding()
	...
	def get_filtered(cls, service, partitionname, filter_) :
		r""" Use this API to fetch filtered set of nspartition_bridgegroup_binding resources.
	...
	def count(cls, service, partitionname) :
		r""" Use this API to count nspartition_bridgegroup_binding resources configued on NetScaler.
		"""
		try :
			obj = nspartition_bridgegroup_binding()
			obj.partitionname = partitionname
	...
	def count_filtered(cls, service, partitionname, filter_) :
		r""" Use this API to count the filtered set of nspartition_bridgegroup_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = nspartition_bridgegroup_binding()
...
class nspartition_bridgegroup_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.nspartition_bridgegroup_binding = [nspartition_bridgegroup_binding() for _ in range(length)]
