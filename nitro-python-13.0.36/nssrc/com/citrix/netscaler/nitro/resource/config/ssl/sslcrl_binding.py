...
class sslcrl_binding(base_resource):
	...
	def sslcrl_serialnumber_bindings(self) :
		r"""serialnumber that can be bound to sslcrl.
		"""
		try :
	...
	@classmethod
	def get(self, service, crlname="", option_="") :
		r""" Use this API to fetch sslcrl_binding resource.
		"""
		try :
			if not crlname :
				obj = sslcrl_binding()
				response = obj.get_resources(service, option_)
			elif type(crlname) is not list :
				obj = sslcrl_binding()
				obj.crlname = crlname
