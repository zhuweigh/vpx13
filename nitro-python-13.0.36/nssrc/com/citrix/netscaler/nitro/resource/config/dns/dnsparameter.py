...
class dnsparameter(base_resource) :
	...
	def cachehitbypass(self) :
		r"""This parameter is applicable only in proxy mode and if this parameter is enabled  we will forward all the client requests to the backend DNS server and the response served will be cached on Citrix ADC.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
	...
	@property
	def maxudppacketsize(self) :
		r"""Maximum UDP packet size that can be handled by Citrix ADC. This is the value advertised by Citrix ADC when responding as an authoritative server and it is also used when Citrix ADC queries other name servers as a forwarder. When acting as a proxy, requests from clients are limited by this parameter - if a request contains a size greater than this value in the OPT record, it will be replaced.<br/>Default value: 1280<br/>Minimum length =  512<br/>Maximum length =  16384.
		"""
		try :
			return self._maxudppacketsize
		except Exception as e:
			raise e

	@maxudppacketsize.setter
	def maxudppacketsize(self, maxudppacketsize) :
		r"""Maximum UDP packet size that can be handled by Citrix ADC. This is the value advertised by Citrix ADC when responding as an authoritative server and it is also used when Citrix ADC queries other name servers as a forwarder. When acting as a proxy, requests from clients are limited by this parameter - if a request contains a size greater than this value in the OPT record, it will be replaced.<br/>Default value: 1280<br/>Minimum length =  512<br/>Maximum length =  16384
		"""
		try :
			self._maxudppacketsize = maxudppacketsize
		except Exception as e:
			raise e
	...
	def update(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				updateresource.maxudppacketsize = resource.maxudppacketsize
