...
class transformpolicylabel(base_resource) :
	...
	@property
	def labelname(self) :
		r"""Name for the policy label. Must begin with a letter, number, or the underscore character (_), and must contain only letters, numbers, and the hyphen (-), period (.) pound (#), space ( ), at (@), equals (=), colon (:), and underscore characters. Can be changed after the URL Transformation policy label is added.
	...
	def policylabeltype(self) :
		r"""Types of transformations allowed by the policies bound to the label. For URL transformation, always http_req (HTTP Request).<br/>Possible values = http_req.
		"""
		try :
	...
	@property
	def newname(self) :
		r"""New name for the policy label.
	...
	def add(cls, client, resource) :
		r""" Use this API to add transformpolicylabel.
		"""
		try :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete transformpolicylabel.
		"""
		try :
			if type(resource) is not list :
	...
	def rename(cls, client, resource, new_labelname) :
		r""" Use this API to rename a transformpolicylabel resource.
		"""
		try :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the transformpolicylabel resources that are configured on netscaler.
		"""
		try :
			if not name :
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of transformpolicylabel resources.
	...
	def count(cls, client) :
		r""" Use this API to count the transformpolicylabel resources configured on NetScaler.
		"""
		try :
			obj = transformpolicylabel()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of transformpolicylabel resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = transformpolicylabel()
