...
class sslcipher_sslprofile_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(sslcipher_sslprofile_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.sslcipher_sslprofile_binding
		except Exception as e :
			raise e
	...
	@classmethod
	def get(cls, service, ciphergroupname="", option_="") :
		r""" Use this API to fetch sslcipher_sslprofile_binding resources.
		"""
		try :
			if not ciphergroupname :
				obj = sslcipher_sslprofile_binding()
	...
	def get_filtered(cls, service, ciphergroupname, filter_) :
		r""" Use this API to fetch filtered set of sslcipher_sslprofile_binding resources.
	...
	def count(cls, service, ciphergroupname) :
		r""" Use this API to count sslcipher_sslprofile_binding resources configued on NetScaler.
		"""
		try :
			obj = sslcipher_sslprofile_binding()
			obj.ciphergroupname = ciphergroupname
	...
	def count_filtered(cls, service, ciphergroupname, filter_) :
		r""" Use this API to count the filtered set of sslcipher_sslprofile_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = sslcipher_sslprofile_binding()
...
class sslcipher_sslprofile_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.sslcipher_sslprofile_binding = [sslcipher_sslprofile_binding() for _ in range(length)]
