...
class transformpolicylabel_transformpolicy_binding(base_resource) :
	...
	@property
	def policyname(self) :
		r"""Name of the URL Transformation policy to bind to the policy label.
		"""
		try :
	...
	@property
	def labelname(self) :
		r"""Name of the URL Transformation policy label to which to bind the policy.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(transformpolicylabel_transformpolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = transformpolicylabel_transformpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [transformpolicylabel_transformpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = transformpolicylabel_transformpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [transformpolicylabel_transformpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, labelname="", option_="") :
		r""" Use this API to fetch transformpolicylabel_transformpolicy_binding resources.
		"""
		try :
			if not labelname :
				obj = transformpolicylabel_transformpolicy_binding()
	...
	def get_filtered(cls, service, labelname, filter_) :
		r""" Use this API to fetch filtered set of transformpolicylabel_transformpolicy_binding resources.
	...
	def count(cls, service, labelname) :
		r""" Use this API to count transformpolicylabel_transformpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = transformpolicylabel_transformpolicy_binding()
			obj.labelname = labelname
	...
	def count_filtered(cls, service, labelname, filter_) :
		r""" Use this API to count the filtered set of transformpolicylabel_transformpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = transformpolicylabel_transformpolicy_binding()
...
class transformpolicylabel_transformpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.transformpolicylabel_transformpolicy_binding = [transformpolicylabel_transformpolicy_binding() for _ in range(length)]
