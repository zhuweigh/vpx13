...
class lbvserver(base_resource) :
	...
	def httpsredirecturl(self) :
		r"""URL to which to redirect traffic if the traffic is recieved from redirect port.
		"""
		try :
			...
			if type(resource) is not list :
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].trofspersistence = resource[i].trofspersistence
						...
						addresources[i].redirectfromport = resource[i].redirectfromport
						addresources[i].httpsredirecturl = resource[i].httpsredirecturl
						addresources[i].retainconnectionsoncluster = resource[i].retainconnectionsoncluster
						addresources[i].adfsproxyprofile = resource[i].adfsproxyprofile
			...
			if type(resource) is not list :
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].trofspersistence = resource[i].trofspersistence
						...
						updateresources[i].redirectfromport = resource[i].redirectfromport
						updateresources[i].httpsredirecturl = resource[i].httpsredirecturl
						updateresources[i].retainconnectionsoncluster = resource[i].retainconnectionsoncluster
						updateresources[i].adfsproxyprofile = resource[i].adfsproxyprofile
