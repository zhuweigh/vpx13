...
class aaagroup(base_resource) :
	...
	@property
	def groupname(self) :
		r"""Name for the group. Must begin with a letter, number, or the underscore character (_), and must consist only of letters, numbers, and the hyphen (-), period (.) pound (#), space ( ), at sign (@), equals (=), colon (:), and underscore  characters. Cannot be changed after the group is added.
		The following requirement applies only to the Citrix ADC CLI:
		If the name includes one or more spaces, enclose the name in double or
		single quotation marks (for example, "my aaa group" or 'my aaa group').<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def weight(self) :
		r"""Weight of this group with respect to other configured aaa groups (lower the number higher the weight).<br/>Default value: 0<br/>Maximum length =  65535.
		"""
		try :
			return self._weight
		except Exception as e:
	...
	def add(cls, client, resource) :
		r""" Use this API to add aaagroup.
		"""
		try :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete aaagroup.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the aaagroup resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = aaagroup()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of aaagroup resources.
	...
	def count(cls, client) :
		r""" Use this API to count the aaagroup resources configured on NetScaler.
		"""
		try :
			obj = aaagroup()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of aaagroup resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaagroup()
