...
class vpnintranetapplication(base_resource) :
	...
	@property
	def hostname(self) :
		r"""Name of the host for which to configure interception. The names are resolved during interception when users log on with the Citrix Gateway Plug-in.<br/>Minimum length =  1.
		"""
		try :
	...
	def interception(self) :
		r"""Interception mode for the intranet application or resource. Correct value depends on the type of client software used to make connections. If the interception mode is set to TRANSPARENT, users connect with the Citrix Gateway Plug-in for Windows. With the PROXY setting, users connect with the Citrix Gateway Plug-in for Java.<br/>Possible values = PROXY, TRANSPARENT.
		"""
		try :
