...
class lsngroup_lsnsipalgprofile_binding(base_resource) :
	""" Binding class showing the lsnsipalgprofile that can be bound to lsngroup.
	"""
	def __init__(self) :
	...
	def sipalgprofilename(self) :
		r"""The name of the LSN SIP ALG Profile.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(lsngroup_lsnsipalgprofile_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.lsngroup_lsnsipalgprofile_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = lsngroup_lsnsipalgprofile_binding()
				updateresource.groupname = resource.groupname
				updateresource.sipalgprofilename = resource.sipalgprofilename
				return updateresource.update_resource(client)
			else :
				if resource and len(resource) > 0 :
					updateresources = [lsngroup_lsnsipalgprofile_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = lsngroup_lsnsipalgprofile_binding()
				deleteresource.groupname = resource.groupname
				deleteresource.sipalgprofilename = resource.sipalgprofilename
				return deleteresource.delete_resource(client)
			else :
				if resource and len(resource) > 0 :
					deleteresources = [lsngroup_lsnsipalgprofile_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, groupname="", option_="") :
		r""" Use this API to fetch lsngroup_lsnsipalgprofile_binding resources.
		"""
		try :
			if not groupname :
				obj = lsngroup_lsnsipalgprofile_binding()
	...
	def get_filtered(cls, service, groupname, filter_) :
		r""" Use this API to fetch filtered set of lsngroup_lsnsipalgprofile_binding resources.
	...
	def count(cls, service, groupname) :
		r""" Use this API to count lsngroup_lsnsipalgprofile_binding resources configued on NetScaler.
		"""
		try :
			obj = lsngroup_lsnsipalgprofile_binding()
			obj.groupname = groupname
	...
	def count_filtered(cls, service, groupname, filter_) :
		r""" Use this API to count the filtered set of lsngroup_lsnsipalgprofile_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = lsngroup_lsnsipalgprofile_binding()
...
class lsngroup_lsnsipalgprofile_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.lsngroup_lsnsipalgprofile_binding = [lsngroup_lsnsipalgprofile_binding() for _ in range(length)]
