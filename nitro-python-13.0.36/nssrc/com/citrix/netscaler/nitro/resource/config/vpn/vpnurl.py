...
class vpnurl(base_resource) :
	...
	def clientlessaccess(self) :
		r"""If clientless access to the resource hosting the link is allowed, also use clientless access for the bookmarked web address in the Secure Client Access based session. Allows single sign-on and other HTTP processing on Citrix Gateway for HTTPS resources.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	@property
	def appjson(self) :
		r"""To store the template details in the json format.
		"""
		try :
			return self._appjson
		except Exception as e:
			raise e

	@appjson.setter
	def appjson(self, appjson) :
		r"""To store the template details in the json format.
		"""
		try :
			self._appjson = appjson
		except Exception as e:
			raise e
	...
	def add(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				addresource.appjson = resource.appjson
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].appjson = resource[i].appjson
	...
	def update(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				updateresource.appjson = resource.appjson
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].appjson = resource[i].appjson
