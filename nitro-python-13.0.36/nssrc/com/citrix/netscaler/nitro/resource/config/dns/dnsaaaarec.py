...
class dnsaaaarec(base_resource) :
	...
	def ipv6address(self) :
		r"""One or more IPv6 addresses to assign to the domain name.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def ecssubnet(self) :
		r"""Subnet for which the cached records need to be removed.
		"""
		try :
	...
	def add(cls, client, resource) :
		r""" Use this API to add dnsaaaarec.
		"""
		try :
	...
	def delete(cls, client, resource) :
		r""" Use this API to delete dnsaaaarec.
		"""
		try :
			if type(resource) is not list :
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the dnsaaaarec resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = dnsaaaarec()
	...
	def get_filtered(cls, client, filter_) :
		r""" Use this API to fetch filtered set of dnsaaaarec resources.
	...
	def count(cls, client) :
		r""" Use this API to count the dnsaaaarec resources configured on NetScaler.
		"""
		try :
			obj = dnsaaaarec()
			option_ = options()
	...
	def count_filtered(cls, client, filter_) :
		r""" Use this API to count filtered the set of dnsaaaarec resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = dnsaaaarec()
