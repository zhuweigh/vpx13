...
class crvserver_feopolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(crvserver_feopolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.crvserver_feopolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = crvserver_feopolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [crvserver_feopolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = crvserver_feopolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [crvserver_feopolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch crvserver_feopolicy_binding resources.
		"""
		try :
			if not name :
				obj = crvserver_feopolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of crvserver_feopolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count crvserver_feopolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = crvserver_feopolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of crvserver_feopolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = crvserver_feopolicy_binding()
...
class crvserver_feopolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.crvserver_feopolicy_binding = [crvserver_feopolicy_binding() for _ in range(length)]
