...
class clusterinstance_clusternode_binding(base_resource) :
	...
	@property
	def nodeid(self) :
		r"""The unique number that identiies a cluster.<br/>Minimum value =  0<br/>Maximum value =  31.
		"""
		try :
			return self._nodeid
	...
	@property
	def state(self) :
		r"""Active, Spare or Passive. An active node serves traffic. A spare node serves as a backup for active nodes. A passive node does not serve traffic. This may be useful during temporary maintenance activity where it is desirable that the node takes part in the consensus protocol, but not serve traffic.<br/>Default value: PASSIVE<br/>Possible values = ACTIVE, SPARE, PASSIVE.
		"""
		try :
			return self._state
		except Exception as e:
	...
	def masterstate(self) :
		r"""Master state.<br/>Possible values = INACTIVE, ACTIVE, UNKNOWN.
		"""
		try :
	...
	@classmethod
	def get(cls, service, clid="", option_="") :
		r""" Use this API to fetch clusterinstance_clusternode_binding resources.
		"""
		try :
			if not clid :
	...
	def get_filtered(cls, service, clid, filter_) :
		r""" Use this API to fetch filtered set of clusterinstance_clusternode_binding resources.
	...
	def count(cls, service, clid) :
		r""" Use this API to count clusterinstance_clusternode_binding resources configued on NetScaler.
		"""
		try :
			obj = clusterinstance_clusternode_binding()
			obj.clid = clid
	...
	def count_filtered(cls, service, clid, filter_) :
		r""" Use this API to count the filtered set of clusterinstance_clusternode_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = clusterinstance_clusternode_binding()
