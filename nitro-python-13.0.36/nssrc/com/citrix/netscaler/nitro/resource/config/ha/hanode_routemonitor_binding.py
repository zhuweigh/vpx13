...
class hanode_routemonitor_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(hanode_routemonitor_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.hanode_routemonitor_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = hanode_routemonitor_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [hanode_routemonitor_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = hanode_routemonitor_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [hanode_routemonitor_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, id="", option_="") :
		r""" Use this API to fetch hanode_routemonitor_binding resources.
		"""
		try :
			if not id :
	...
	def get_filtered(cls, service, id, filter_) :
		r""" Use this API to fetch filtered set of hanode_routemonitor_binding resources.
	...
	def count(cls, service, id) :
		r""" Use this API to count hanode_routemonitor_binding resources configued on NetScaler.
		"""
		try :
			obj = hanode_routemonitor_binding()
			obj.id = id
	...
	def count_filtered(cls, service, id, filter_) :
		r""" Use this API to count the filtered set of hanode_routemonitor_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = hanode_routemonitor_binding()
...
class hanode_routemonitor_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.hanode_routemonitor_binding = [hanode_routemonitor_binding() for _ in range(length)]
