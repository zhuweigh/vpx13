...
class authenticationtacacsaction(base_resource) :
	...
	def authtimeout(self) :
		r"""Number of seconds the Citrix ADC waits for a response from the TACACS+ server.<br/>Default value: 3<br/>Minimum length =  1.
		"""
		try :
	...
	def tacacssecret(self) :
		r"""Key shared between the TACACS+ server and the Citrix ADC. 
		Required for allowing the Citrix ADC to communicate with the TACACS+ server.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def attributes(self) :
		r"""List of attribute names separated by ',' which needs to be fetched from tacacs server. 
