...
class sslvserver_binding(base_resource):
	...
	def vservername(self) :
		r"""Name of the SSL virtual server for which to show detailed information.<br/>Minimum length =  1.
		"""
		try :
	...
	def sslvserver_sslciphersuite_bindings(self) :
		r"""sslciphersuite that can be bound to sslvserver.
		"""
		try :
	...
	def sslvserver_ecccurve_bindings(self) :
		r"""ecccurve that can be bound to sslvserver.
		"""
		try :
	...
	def sslvserver_sslpolicy_bindings(self) :
		r"""sslpolicy that can be bound to sslvserver.
		"""
		try :
	...
	def sslvserver_sslcertkey_bindings(self) :
		r"""sslcertkey that can be bound to sslvserver.
		"""
		try :
	...
	def sslvserver_sslcipher_bindings(self) :
		r"""sslcipher that can be bound to sslvserver.
		"""
		try :
	...
	@classmethod
	def get(self, service, vservername="", option_="") :
		r""" Use this API to fetch sslvserver_binding resource.
		"""
		try :
			if not vservername :
				obj = sslvserver_binding()
				response = obj.get_resources(service, option_)
			elif type(vservername) is not list :
				obj = sslvserver_binding()
