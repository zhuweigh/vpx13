...
class gslbvserver_spilloverpolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(gslbvserver_spilloverpolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = gslbvserver_spilloverpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [gslbvserver_spilloverpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = gslbvserver_spilloverpolicy_binding()
				deleteresource.name = resource.name
				deleteresource.policyname = resource.policyname
				return deleteresource.delete_resource(client)
			else :
				if resource and len(resource) > 0 :
					deleteresources = [gslbvserver_spilloverpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch gslbvserver_spilloverpolicy_binding resources.
		"""
		try :
			if not name :
				obj = gslbvserver_spilloverpolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of gslbvserver_spilloverpolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count gslbvserver_spilloverpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = gslbvserver_spilloverpolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of gslbvserver_spilloverpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = gslbvserver_spilloverpolicy_binding()
...
class gslbvserver_spilloverpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.gslbvserver_spilloverpolicy_binding = [gslbvserver_spilloverpolicy_binding() for _ in range(length)]
