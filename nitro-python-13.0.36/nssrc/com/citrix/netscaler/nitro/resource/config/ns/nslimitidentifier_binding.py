...
class nslimitidentifier_binding(base_resource):
	...
	def nslimitidentifier_nslimitsessions_bindings(self) :
		r"""nslimitsessions that can be bound to nslimitidentifier.
		"""
		try :
	...
	@classmethod
	def get(self, service, limitidentifier="", option_="") :
		r""" Use this API to fetch nslimitidentifier_binding resource.
		"""
		try :
			if not limitidentifier :
				obj = nslimitidentifier_binding()
				response = obj.get_resources(service, option_)
			elif type(limitidentifier) is not list :
				obj = nslimitidentifier_binding()
