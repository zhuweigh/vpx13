	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = appfwprofile_csrftag_binding()
				...
				if resource and len(resource) > 0 :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].csrftag = resource[i].csrftag
						updateresources[i].csrfformactionurl = resource[i].csrfformactionurl
			...
			if resource and type(resource) is not list :
				...
				if resource and len(resource) > 0 :
					...
					for i in range(len(resource)) :
						...
						deleteresources[i].csrfformactionurl = resource[i].csrfformactionurl
