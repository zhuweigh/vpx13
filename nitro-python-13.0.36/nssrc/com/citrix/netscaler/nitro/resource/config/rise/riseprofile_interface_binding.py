...
class riseprofile_interface_binding(base_resource) :
	...
	"""
	def __init__(self) :
		self._memberinterface = None
		self._profilename = None
		self.___count = None

	@property
	def memberinterface(self) :
		r"""RISE profile member interfaces.
		"""
		try :
	...
	@classmethod
	def get(cls, service, profilename="", option_="") :
		r""" Use this API to fetch riseprofile_interface_binding resources.
		"""
		try :
			if not profilename :
	...
	def get_filtered(cls, service, profilename, filter_) :
		r""" Use this API to fetch filtered set of riseprofile_interface_binding resources.
	...
	def count(cls, service, profilename) :
		r""" Use this API to count riseprofile_interface_binding resources configued on NetScaler.
		"""
		try :
			obj = riseprofile_interface_binding()
			obj.profilename = profilename
	...
	def count_filtered(cls, service, profilename, filter_) :
		r""" Use this API to count the filtered set of riseprofile_interface_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = riseprofile_interface_binding()
