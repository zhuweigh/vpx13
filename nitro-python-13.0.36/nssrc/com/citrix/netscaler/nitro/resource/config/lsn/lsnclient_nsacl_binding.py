...
class lsnclient_nsacl_binding(base_resource) :
	...
	def aclname(self) :
		...
		The condition specified in the extended ACL rule identifies the traffic from an LSN subscriber for which the Citrix ADC is to perform large scale NAT. .<br/>Minimum length =  1.
		"""
		try :
