...
class hasync(base_resource) :
	...
	@property
	def force(self) :
		r"""Force synchronization regardless of the state of HA propagation and HA synchronization on either node.
		"""
		try :
			return self._force
	...
	@property
	def save(self) :
		r"""After synchronization, automatically save the configuration in the secondary node configuration file (ns.conf) without prompting for confirmation.<br/>Possible values = YES, NO.
		"""
		try :
			return self._save
		except Exception as e:
	...
	def Force(cls, client, resource) :
		r""" Use this API to Force hasync.
		"""
		try :
