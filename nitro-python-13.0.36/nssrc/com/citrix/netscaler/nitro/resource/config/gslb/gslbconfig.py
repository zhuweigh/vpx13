...
class gslbconfig(base_resource) :
	...
	def __init__(self) :
		self._preview = None
		self._debug = None
		self._forcesync = None
		self._nowarn = None
		self._saveconfig = None
		self._command = None

	@property
	def preview(self) :
		r"""Do not synchronize the GSLB sites, but display the commands that would be applied on the slave node upon synchronization. Mutually exclusive with the Save Configuration option.
		"""
		try :
	...
	@property
	def debug(self) :
		r"""Generate verbose output when synchronizing the GSLB sites. The Debug option generates more verbose output than the sync gslb config command in which the option is not used, and is useful for analyzing synchronization issues.
		"""
		try :
			return self._debug
		except Exception as e:
	...
	@property
	def nowarn(self) :
		r"""Suppress the warning and the confirmation prompt that are displayed before site synchronization begins. This option can be used in automation scripts that must not be interrupted by a prompt.
		"""
		try :
			return self._nowarn
		except Exception as e:
	...
	@property
	def command(self) :
		r"""Run the specified command on the master node and then on all the slave nodes. You cannot use this option with the force sync and preview options.
		"""
		try :
	...
	def sync(cls, client, resource) :
		r""" Use this API to sync gslbconfig.
		"""
		try :
