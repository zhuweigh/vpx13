...
class hanode_fis_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(hanode_fis_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.hanode_fis_binding
		except Exception as e :
			raise e
	...
	def get(cls, service, id="", option_="") :
		r""" Use this API to fetch hanode_fis_binding resources.
		"""
		try :
			if not id :
				obj = hanode_fis_binding()
	...
	def get_filtered(cls, service, id, filter_) :
		r""" Use this API to fetch filtered set of hanode_fis_binding resources.
	...
	def count(cls, service, id) :
		r""" Use this API to count hanode_fis_binding resources configued on NetScaler.
		"""
		try :
			obj = hanode_fis_binding()
			obj.id = id
	...
	def count_filtered(cls, service, id, filter_) :
		r""" Use this API to count the filtered set of hanode_fis_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = hanode_fis_binding()
...
class hanode_fis_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.hanode_fis_binding = [hanode_fis_binding() for _ in range(length)]
