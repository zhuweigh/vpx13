...
class lbvserver_capolicy_binding(base_resource) :
	""" Binding class showing the capolicy that can be bound to lbvserver.
	"""
	def __init__(self) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(lbvserver_capolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.lbvserver_capolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = lbvserver_capolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [lbvserver_capolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = lbvserver_capolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [lbvserver_capolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch lbvserver_capolicy_binding resources.
		"""
		try :
			if not name :
				obj = lbvserver_capolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of lbvserver_capolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count lbvserver_capolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = lbvserver_capolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of lbvserver_capolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = lbvserver_capolicy_binding()
...
class lbvserver_capolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.lbvserver_capolicy_binding = [lbvserver_capolicy_binding() for _ in range(length)]
