...
class authenticationldapaction(base_resource) :
	...
	def __init__(self) :
		...
		self._sshpublickey = None
		self._pushservice = None
		self._otpsecret = None
		self._email = None
		self._kbattribute = None
		self._alternateemailattr = None
		self._success = None
		self._failure = None
		self.___count = None
	...
	@property
	def sectype(self) :
		r"""Type of security used for communications between the Citrix ADC and the LDAP server. For the PLAINTEXT setting, no encryption is required.<br/>Default value: PLAINTEXT<br/>Possible values = PLAINTEXT, TLS, SSL.
		"""
		try :
	...
	def requireuser(self) :
		r"""Require a successful user search for authentication.
		CAUTION!  This field should be set to NO only if usersearch not required [Both username validation as well as password validation skipped] and (non-LDAP) authentication methods are in use and either bound to a primary list or flagged as secondary.<br/>Default value: YES<br/>Possible values = YES, NO.
		"""
		try :
	...
	def nestedgroupextraction(self) :
		r"""Allow nested group extraction, in which the Citrix ADC queries external LDAP servers to determine whether a group is part of another group.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	@property
	def attributes(self) :
		r"""List of attribute names separated by ',' which needs to be fetched from ldap server. 
	...
	@property
	def sshpublickey(self) :
		r"""SSH PublicKey is attribute on AD. This attribute is used to retrieve ssh PublicKey for RBA authentication.<br/>Minimum length =  1.
		"""
		try :
			return self._sshpublickey
		except Exception as e:
			raise e

	@sshpublickey.setter
	def sshpublickey(self, sshpublickey) :
		r"""SSH PublicKey is attribute on AD. This attribute is used to retrieve ssh PublicKey for RBA authentication.<br/>Minimum length =  1
		"""
		try :
			self._sshpublickey = sshpublickey
		except Exception as e:
			raise e
	...
	@property
	def otpsecret(self) :
		r"""OneTimePassword(OTP) Secret key attribute on AD. This attribute is used to store and retrieve secret key used for OTP check.<br/>Minimum length =  1.
		"""
		try :
	...
	@otpsecret.setter
	def otpsecret(self, otpsecret) :
		r"""OneTimePassword(OTP) Secret key attribute on AD. This attribute is used to store and retrieve secret key used for OTP check.<br/>Minimum length =  1
		"""
		try :
			self._otpsecret = otpsecret
		except Exception as e:
			raise e

	@property
	def email(self) :
		r"""The Citrix ADC uses the email attribute to query the Active Directory for the email id of a user.<br/>Default value: mail<br/>Maximum length =  128.
		"""
		try :
			return self._email
		except Exception as e:
	...
	@property
	def kbattribute(self) :
		r"""KnowledgeBasedAuthentication(KBA) attribute on AD. This attribute is used to store and retrieve preconfigured Question and Answer knowledge base used for KBA authentication.<br/>Minimum length =  1<br/>Maximum length =  127.
		"""
		try :
			return self._kbattribute
		except Exception as e:
			raise e

	@kbattribute.setter
	def kbattribute(self, kbattribute) :
		r"""KnowledgeBasedAuthentication(KBA) attribute on AD. This attribute is used to store and retrieve preconfigured Question and Answer knowledge base used for KBA authentication.<br/>Minimum length =  1<br/>Maximum length =  127
		"""
		try :
			self._kbattribute = kbattribute
		except Exception as e:
			raise e

	@property
	def alternateemailattr(self) :
		r"""The NetScaler appliance uses the alternateive email attribute to query the Active Directory for the alternative email id of a user.<br/>Maximum length =  128.
		"""
		try :
			return self._alternateemailattr
		except Exception as e:
			raise e

	@alternateemailattr.setter
	def alternateemailattr(self, alternateemailattr) :
		r"""The NetScaler appliance uses the alternateive email attribute to query the Active Directory for the alternative email id of a user.<br/>Maximum length =  128
		"""
		try :
			self._alternateemailattr = alternateemailattr
		except Exception as e:
			raise e
	...
	def add(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				addresource.sshpublickey = resource.sshpublickey
				addresource.pushservice = resource.pushservice
				addresource.otpsecret = resource.otpsecret
				addresource.email = resource.email
				addresource.kbattribute = resource.kbattribute
				addresource.alternateemailattr = resource.alternateemailattr
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].sshpublickey = resource[i].sshpublickey
						addresources[i].pushservice = resource[i].pushservice
						addresources[i].otpsecret = resource[i].otpsecret
						addresources[i].email = resource[i].email
						addresources[i].kbattribute = resource[i].kbattribute
						addresources[i].alternateemailattr = resource[i].alternateemailattr
	...
	def update(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				updateresource.otpsecret = resource.otpsecret
				updateresource.sshpublickey = resource.sshpublickey
				updateresource.pushservice = resource.pushservice
				updateresource.email = resource.email
				updateresource.kbattribute = resource.kbattribute
				updateresource.alternateemailattr = resource.alternateemailattr
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						updateresources[i].otpsecret = resource[i].otpsecret
						updateresources[i].sshpublickey = resource[i].sshpublickey
						updateresources[i].pushservice = resource[i].pushservice
						updateresources[i].email = resource[i].email
						updateresources[i].kbattribute = resource[i].kbattribute
						updateresources[i].alternateemailattr = resource[i].alternateemailattr
