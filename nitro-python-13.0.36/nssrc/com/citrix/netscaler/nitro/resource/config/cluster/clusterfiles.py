...
class clusterfiles(base_resource) :
	...
	@property
	def mode(self) :
		r"""The directories and files to be synchronized. The available settings function as follows:
		Mode    Paths
		all           /nsconfig/ssl/
		/var/netscaler/ssl/
		/var/vpn/bookmark/
		...
		Default value: all.<br/>Possible values = all, bookmarks, ssl, htmlinjection, imports, misc, dns, krb, AAA, app_catalog, all_plus_misc, all_minus_misc.
		"""
		try :
			return self._mode
	...
	def sync(cls, client, resource) :
		r""" Use this API to sync clusterfiles.
		"""
		try :
