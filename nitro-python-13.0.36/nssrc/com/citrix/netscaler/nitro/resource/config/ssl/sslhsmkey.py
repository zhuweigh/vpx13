...
class sslhsmkey(base_resource) :
	...
	@property
	def hsmtype(self) :
		r"""Type of HSM.<br/>Default value: THALES<br/>Possible values = THALES, SAFENET, KEYVAULT.
		"""
		try :
	...
	@property
	def keystore(self) :
		r"""Name of keystore object representing HSM where key is stored. For example, name of keyvault object or azurekeyvault authentication object. Applies only to KEYVAULT type HSM.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def state(self) :
		r"""Current state of key.<br/>Possible values = Created, Access Token Unavailable, Unauthorized, Does not exist, Unreachable, Marked down, Key operations successful, Key operations failed, Key operation throttled.
		"""
		try :
			return self._state
		except Exception as e:
	...
	def add(cls, client, resource) :
		...
		try :
			if type(resource) is not list :
				...
				addresource.keystore = resource.keystore
				...
				if (resource and len(resource) > 0) :
					...
					for i in range(len(resource)) :
						...
						addresources[i].keystore = resource[i].keystore
	...
	class State:
		Created = "Created"
		Access_Token_Unavailable = "Access Token Unavailable"
		Unauthorized = "Unauthorized"
		Does_not_exist = "Does not exist"
		Unreachable = "Unreachable"
		Marked_down = "Marked down"
		Key_operations_successful = "Key operations successful"
		Key_operations_failed = "Key operations failed"
		Key_operation_throttled = "Key operation throttled"
