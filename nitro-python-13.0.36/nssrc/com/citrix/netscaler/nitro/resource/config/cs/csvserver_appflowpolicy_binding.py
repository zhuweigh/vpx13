...
class csvserver_appflowpolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(csvserver_appflowpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.csvserver_appflowpolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = csvserver_appflowpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [csvserver_appflowpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = csvserver_appflowpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [csvserver_appflowpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch csvserver_appflowpolicy_binding resources.
		"""
		try :
			if not name :
				obj = csvserver_appflowpolicy_binding()
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of csvserver_appflowpolicy_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count csvserver_appflowpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = csvserver_appflowpolicy_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of csvserver_appflowpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = csvserver_appflowpolicy_binding()
...
class csvserver_appflowpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.csvserver_appflowpolicy_binding = [csvserver_appflowpolicy_binding() for _ in range(length)]
