...
class wisite_binding(base_resource):
	...
	@property
	def sitepath(self) :
		r"""Path of a Web Interface site whose details you want the Citrix ADC to display.<br/>Minimum length =  1<br/>Maximum length =  250.
		"""
		try :
	...
	def wisite_translationinternalip_bindings(self) :
		r"""translationinternalip that can be bound to wisite.
		"""
		try :
	...
	def wisite_accessmethod_bindings(self) :
		r"""accessmethod that can be bound to wisite.
		"""
		try :
	...
	def wisite_farmname_bindings(self) :
		r"""farmname that can be bound to wisite.
		"""
		try :
	...
	@classmethod
	def get(self, service, sitepath="", option_="") :
		r""" Use this API to fetch wisite_binding resource.
		"""
		try :
			if not sitepath :
				obj = wisite_binding()
				response = obj.get_resources(service, option_)
			elif type(sitepath) is not list :
				obj = wisite_binding()
