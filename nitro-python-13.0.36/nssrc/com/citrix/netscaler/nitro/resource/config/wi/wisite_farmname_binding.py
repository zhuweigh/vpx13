...
class wisite_farmname_binding(base_resource) :
	...
	def __init__(self) :
		self._farmname = None
		self._xmlserveraddresses = None
		self._xmlport = None
		self._transport = None
		self._sslrelayport = None
		self._loadbalance = None
		self._groups = None
		self._recoveryfarm = None
		self._sitepath = None
		self.___count = None

	@property
	def sitepath(self) :
		r"""Path to the Web Interface site.<br/>Minimum length =  1<br/>Maximum length =  250.
		"""
		try :
	...
	@property
	def groups(self) :
		r"""Active Directory groups that are permitted to enumerate resources from server farms. Including a setting for this parameter activates the user roaming feature. A maximum of 512 user groups can be specified for each farm defined with the Farm<n> parameter.  The groups must be comma separated.
		"""
		try :
			return self._groups
		except Exception as e:
	...
	@property
	def xmlport(self) :
		r"""Port number at which to contact the XML service.
		"""
		try :
	...
	@property
	def transport(self) :
		r"""Transport protocol to use for transferring data, related to the Web Interface site, between the Citrix ADC and the XML service.<br/>Possible values = HTTP, HTTPS, SSLRELAY.
		"""
		try :
	...
	def sslrelayport(self) :
		r"""TCP port at which the XenApp or XenDesktop servers listenfor SSL Relay traffic from the Citrix ADC. This parameter is required if you have set SSL Relay as the transport protocol. 
	...
	@property
	def farmname(self) :
		r"""Name for the logical representation of a XenApp or XenDesktop farm to be bound to the Web Interface site. Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters.
		"""
		try :
	...
	def loadbalance(self) :
		r"""Use all the XML servers (load balancing mode) or only one server (failover mode).<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def recoveryfarm(self) :
		r"""Binded farm is set as a recovery farm.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def xmlserveraddresses(self) :
		r"""Comma-separated IP addresses or host names of XenApp or XenDesktop servers providing XML services.
		"""
		try :
	...
	@classmethod
	def get(cls, service, sitepath="", option_="") :
		r""" Use this API to fetch wisite_farmname_binding resources.
		"""
		try :
			if not sitepath :
	...
	def get_filtered(cls, service, sitepath, filter_) :
		r""" Use this API to fetch filtered set of wisite_farmname_binding resources.
	...
	def count(cls, service, sitepath) :
		r""" Use this API to count wisite_farmname_binding resources configued on NetScaler.
		"""
		try :
			obj = wisite_farmname_binding()
			obj.sitepath = sitepath
	...
	def count_filtered(cls, service, sitepath, filter_) :
		r""" Use this API to count the filtered set of wisite_farmname_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = wisite_farmname_binding()
