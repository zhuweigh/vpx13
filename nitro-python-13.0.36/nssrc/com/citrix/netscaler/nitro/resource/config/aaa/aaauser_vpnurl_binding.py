...
class aaauser_vpnurl_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(aaauser_vpnurl_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = aaauser_vpnurl_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [aaauser_vpnurl_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = aaauser_vpnurl_binding()
				deleteresource.username = resource.username
				deleteresource.urlname = resource.urlname
				return deleteresource.delete_resource(client)
			else :
				if resource and len(resource) > 0 :
					deleteresources = [aaauser_vpnurl_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, username="", option_="") :
		r""" Use this API to fetch aaauser_vpnurl_binding resources.
		"""
		try :
			if not username :
				obj = aaauser_vpnurl_binding()
	...
	def get_filtered(cls, service, username, filter_) :
		r""" Use this API to fetch filtered set of aaauser_vpnurl_binding resources.
	...
	def count(cls, service, username) :
		r""" Use this API to count aaauser_vpnurl_binding resources configued on NetScaler.
		"""
		try :
			obj = aaauser_vpnurl_binding()
			obj.username = username
	...
	def count_filtered(cls, service, username, filter_) :
		r""" Use this API to count the filtered set of aaauser_vpnurl_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaauser_vpnurl_binding()
...
class aaauser_vpnurl_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.aaauser_vpnurl_binding = [aaauser_vpnurl_binding() for _ in range(length)]
