...
class vpnglobal_authenticationradiuspolicy_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(vpnglobal_authenticationradiuspolicy_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = vpnglobal_authenticationradiuspolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [vpnglobal_authenticationradiuspolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = vpnglobal_authenticationradiuspolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [vpnglobal_authenticationradiuspolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	@classmethod
	def get(cls, service) :
		r""" Use this API to fetch a vpnglobal_authenticationradiuspolicy_binding resources.
		"""
		try :
			obj = vpnglobal_authenticationradiuspolicy_binding()
	...
	def get_filtered(cls, service, filter_) :
		r""" Use this API to fetch filtered set of vpnglobal_authenticationradiuspolicy_binding resources.
	...
	def count(cls, service) :
		r""" Use this API to count vpnglobal_authenticationradiuspolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = vpnglobal_authenticationradiuspolicy_binding()
			option_ = options()
	...
	def count_filtered(cls, service, filter_) :
		r""" Use this API to count the filtered set of vpnglobal_authenticationradiuspolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vpnglobal_authenticationradiuspolicy_binding()
...
class vpnglobal_authenticationradiuspolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.vpnglobal_authenticationradiuspolicy_binding = [vpnglobal_authenticationradiuspolicy_binding() for _ in range(length)]
