...
class arpparam(base_resource) :
	...
	def __init__(self) :
		self._timeout = None
		self._spoofvalidation = None

	@property
	def timeout(self) :
		r"""Time-out value (aging time) for the dynamically learned ARP entries, in seconds. The new value applies only to ARP entries that are dynamically learned after the new value is set. Previously existing ARP entries expire after the previously configured aging time.<br/>Default value: 1200<br/>Minimum length =  5<br/>Maximum length =  1200.
		"""
		try :
	...
	def spoofvalidation(self) :
		r"""enable/disable arp spoofing validation.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
	...
	def update(cls, client, resource) :
		r""" Use this API to update arpparam.
		"""
		try :
	...
	def unset(cls, client, resource, args) :
		r""" Use this API to unset the properties of arpparam resource.
	...
	def get(cls, client, name="", option_="") :
		r""" Use this API to fetch all the arpparam resources that are configured on netscaler.
		"""
		try :
			if not name :
				obj = arpparam()
