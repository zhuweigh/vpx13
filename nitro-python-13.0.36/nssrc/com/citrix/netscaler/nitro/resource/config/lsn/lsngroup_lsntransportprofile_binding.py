...
class lsngroup_lsntransportprofile_binding(base_resource) :
	""" Binding class showing the lsntransportprofile that can be bound to lsngroup.
	"""
	def __init__(self) :
	...
	def transportprofilename(self) :
		r"""Name of the LSN transport profile to bind to the specified LSN group. Bind a profile for each protocol for which you want to specify settings.
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(lsngroup_lsntransportprofile_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.lsngroup_lsntransportprofile_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = lsngroup_lsntransportprofile_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [lsngroup_lsntransportprofile_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = lsngroup_lsntransportprofile_binding()
				deleteresource.groupname = resource.groupname
				deleteresource.transportprofilename = resource.transportprofilename
				return deleteresource.delete_resource(client)
			else :
				if resource and len(resource) > 0 :
					deleteresources = [lsngroup_lsntransportprofile_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, groupname="", option_="") :
		r""" Use this API to fetch lsngroup_lsntransportprofile_binding resources.
		"""
		try :
			if not groupname :
				obj = lsngroup_lsntransportprofile_binding()
	...
	def get_filtered(cls, service, groupname, filter_) :
		r""" Use this API to fetch filtered set of lsngroup_lsntransportprofile_binding resources.
	...
	def count(cls, service, groupname) :
		r""" Use this API to count lsngroup_lsntransportprofile_binding resources configued on NetScaler.
		"""
		try :
			obj = lsngroup_lsntransportprofile_binding()
			obj.groupname = groupname
	...
	def count_filtered(cls, service, groupname, filter_) :
		r""" Use this API to count the filtered set of lsngroup_lsntransportprofile_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = lsngroup_lsntransportprofile_binding()
...
class lsngroup_lsntransportprofile_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.lsngroup_lsntransportprofile_binding = [lsngroup_lsntransportprofile_binding() for _ in range(length)]
