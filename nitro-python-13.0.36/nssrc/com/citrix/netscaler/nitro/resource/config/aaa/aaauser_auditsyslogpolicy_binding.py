...
class aaauser_auditsyslogpolicy_binding(base_resource) :
	""" Binding class showing the auditsyslogpolicy that can be bound to aaauser.
	"""
	def __init__(self) :
		self._policy = None
		self._priority = None
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(aaauser_auditsyslogpolicy_binding_response, response, self.__class__.__name__)
			if(result.errorcode != 0) :
				...
				if result.severity :
					...
					raise nitro_exception(result.errorcode, str(result.message), str(result.severity))
			return result.aaauser_auditsyslogpolicy_binding
		except Exception as e :
			raise e
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = aaauser_auditsyslogpolicy_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [aaauser_auditsyslogpolicy_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = aaauser_auditsyslogpolicy_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [aaauser_auditsyslogpolicy_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, username="", option_="") :
		r""" Use this API to fetch aaauser_auditsyslogpolicy_binding resources.
		"""
		try :
			if not username :
				obj = aaauser_auditsyslogpolicy_binding()
	...
	def get_filtered(cls, service, username, filter_) :
		r""" Use this API to fetch filtered set of aaauser_auditsyslogpolicy_binding resources.
	...
	def count(cls, service, username) :
		r""" Use this API to count aaauser_auditsyslogpolicy_binding resources configued on NetScaler.
		"""
		try :
			obj = aaauser_auditsyslogpolicy_binding()
			obj.username = username
	...
	def count_filtered(cls, service, username, filter_) :
		r""" Use this API to count the filtered set of aaauser_auditsyslogpolicy_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = aaauser_auditsyslogpolicy_binding()
...
class aaauser_auditsyslogpolicy_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.aaauser_auditsyslogpolicy_binding = [aaauser_auditsyslogpolicy_binding() for _ in range(length)]
