...
class cspolicy_crvserver_binding(base_resource) :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(cspolicy_crvserver_binding_response, response, self.__class__.__name__)
	...
	def get(cls, service, policyname="", option_="") :
		r""" Use this API to fetch cspolicy_crvserver_binding resources.
		"""
		try :
			if not policyname :
				obj = cspolicy_crvserver_binding()
	...
	def get_filtered(cls, service, policyname, filter_) :
		r""" Use this API to fetch filtered set of cspolicy_crvserver_binding resources.
	...
	def count(cls, service, policyname) :
		r""" Use this API to count cspolicy_crvserver_binding resources configued on NetScaler.
		"""
		try :
			obj = cspolicy_crvserver_binding()
			obj.policyname = policyname
	...
	def count_filtered(cls, service, policyname, filter_) :
		r""" Use this API to count the filtered set of cspolicy_crvserver_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = cspolicy_crvserver_binding()
...
class cspolicy_crvserver_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.cspolicy_crvserver_binding = [cspolicy_crvserver_binding() for _ in range(length)]
