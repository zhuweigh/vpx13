...
class gslbvserver_domain_binding(base_resource) :
	...
	def __init__(self) :
		...
		self._name = None
		self._backupipflag = None
		self._cookie_domainflag = None
		self.___count = None

	@property
	def backupipflag(self) :
		r"""The IP address of the backup service for the specified domain name. Used when all the services bound to the domain are down, or when the backup chain of virtual servers is down.
		"""
		try :
	...
	def cookietimeout(self) :
		r"""Timeout, in minutes, for the GSLB site cookie.<br/>Minimum value =  0<br/>Maximum value =  1440.
		"""
		try :
	...
	@property
	def ttl(self) :
		r"""Time to live (TTL) for the domain.<br/>Minimum value =  1.
		"""
		try :
			return self._ttl
		except Exception as e:
	...
	def sitedomainttl(self) :
		r"""TTL, in seconds, for all internally created site domains (created when a site prefix is configured on a GSLB service) that are associated with this virtual server.<br/>Minimum value =  1.
		"""
		try :
	...
	def cookie_domainflag(self) :
		r"""The cookie domain for the GSLB site. Used when inserting the GSLB site cookie in the HTTP response.
		"""
		try :
	...
	def get(cls, service, name="", option_="") :
		r""" Use this API to fetch gslbvserver_domain_binding resources.
		"""
		try :
			if not name :
	...
	def get_filtered(cls, service, name, filter_) :
		r""" Use this API to fetch filtered set of gslbvserver_domain_binding resources.
	...
	def count(cls, service, name) :
		r""" Use this API to count gslbvserver_domain_binding resources configued on NetScaler.
		"""
		try :
			obj = gslbvserver_domain_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		r""" Use this API to count the filtered set of gslbvserver_domain_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = gslbvserver_domain_binding()
