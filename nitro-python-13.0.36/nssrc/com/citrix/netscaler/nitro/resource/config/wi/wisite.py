...
class wisite(base_resource) :
	...
	def __init__(self) :
		...
		self._wiauthenticationmethods = None
	...
	@property
	def sitepath(self) :
		r"""Path to the Web Interface site being created on the Citrix ADC.<br/>Minimum length =  1<br/>Maximum length =  250.
		"""
		try :
	...
	def websessiontimeout(self) :
		r"""Time-out, in minutes, for idle Web Interface browser sessions. If a client's session is idle for a time that exceeds the time-out value, the Citrix ADC terminates the connection.<br/>Default value: 20<br/>Minimum length =  1<br/>Maximum length =  1440.
		"""
		try :
	...
	def defaultaccessmethod(self) :
		...
		Note: In the Citrix ADC command line, mapping entries can be created by using the bind wi site command.<br/>Possible values = Direct, Alternate, Translated, GatewayDirect, GatewayAlternate, GatewayTranslated.
		"""
		try :
