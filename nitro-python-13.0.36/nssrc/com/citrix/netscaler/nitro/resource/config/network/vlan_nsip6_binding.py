...
class vlan_nsip6_binding(base_resource) :
	...
	@property
	def netmask(self) :
		r"""Subnet mask for the network address defined for this VLAN.
		"""
		try :
	...
	@property
	def ipaddress(self) :
		r"""The IP address assigned to the VLAN.
		"""
		try :
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(vlan_nsip6_binding_response, response, self.__class__.__name__)
	...
	def add(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				updateresource = vlan_nsip6_binding()
				...
				if resource and len(resource) > 0 :
					updateresources = [vlan_nsip6_binding() for _ in range(len(resource))]
	...
	def delete(cls, client, resource) :
		try :
			if resource and type(resource) is not list :
				deleteresource = vlan_nsip6_binding()
				...
				if resource and len(resource) > 0 :
					deleteresources = [vlan_nsip6_binding() for _ in range(len(resource))]
					for i in range(len(resource)) :
	...
	def get(cls, service, id="", option_="") :
		r""" Use this API to fetch vlan_nsip6_binding resources.
		"""
		try :
			if not id :
				obj = vlan_nsip6_binding()
	...
	def get_filtered(cls, service, id, filter_) :
		r""" Use this API to fetch filtered set of vlan_nsip6_binding resources.
	...
	def count(cls, service, id) :
		r""" Use this API to count vlan_nsip6_binding resources configued on NetScaler.
		"""
		try :
			obj = vlan_nsip6_binding()
			obj.id = id
	...
	def count_filtered(cls, service, id, filter_) :
		r""" Use this API to count the filtered set of vlan_nsip6_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = vlan_nsip6_binding()
...
class vlan_nsip6_binding_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.vlan_nsip6_binding = [vlan_nsip6_binding() for _ in range(length)]
