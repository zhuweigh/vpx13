...
class icapolicy_stats(base_resource) :
	r""" Statistics for ICA policy resource.
	"""
	def __init__(self) :
		self._name = None
	...
	@property
	def name(self) :
		r"""Name of the ica policy.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(icapolicy_response, response, self.__class__.__name__.replace('_stats',''))
	...
	def  get(cls, service, name="", option_="") :
		r""" Use this API to fetch the statistics of all icapolicy_stats resources that are configured on netscaler.
...
class icapolicy_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.icapolicy = [icapolicy_stats() for _ in range(length)]
