...
class hanode_stats(base_resource) :
	r""" Statistics for node resource.
	"""
	def __init__(self) :
	...
	def hacurstatus(self) :
		r"""Whether a Citrix ADC is configured for high availability. Possible values are YES and NO. If the value is NO, the high availability statistics below are invalid.
		"""
		try :
	...
	@property
	def hatotpkttx(self) :
		r"""Number of heartbeat packets sent to the peer node. Heartbeats are sent at regular intervals (default is 200 milliseconds) to determine the state of the peer node.
		"""
		try :
	...
	def hapktrxrate(self) :
		r"""Rate (/s) counter for hatotpktrx.
		"""
		try :
	...
	def haerrproptimeout(self) :
		r"""Number of times propagation timed out.
		"""
		try :
	...
	def hapkttxrate(self) :
		r"""Rate (/s) counter for hatotpkttx.
		"""
		try :
	...
	@property
	def hatotpktrx(self) :
		r"""Number of heartbeat packets received from the peer node. Heartbeats are sent at regular intervals (default is 200 milliseconds) to determine the state of the peer node.
		"""
		try :
	...
	def  get(cls, service, name="", option_="") :
		r""" Use this API to fetch the statistics of all hanode_stats resources that are configured on netscaler.
