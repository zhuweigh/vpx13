...
class tunneltrafficpolicy_stats(base_resource) :
	r""" Statistics for tunnel policy resource.
	"""
	def __init__(self) :
		self._name = None
	...
	@property
	def name(self) :
		r"""Name of the advanced tunnel traffic policy.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		r"""Name of the advanced tunnel traffic policy.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(tunneltrafficpolicy_response, response, self.__class__.__name__.replace('_stats',''))
	...
	def  get(cls, service, name="", option_="") :
		r""" Use this API to fetch the statistics of all tunneltrafficpolicy_stats resources that are configured on netscaler.
		 set statbindings=True in options to retrieve bindings.
		"""
		try :
			obj = tunneltrafficpolicy_stats()
			if not name :
...
class tunneltrafficpolicy_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.tunneltrafficpolicy = [tunneltrafficpolicy_stats() for _ in range(length)]
