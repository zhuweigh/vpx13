...
class audit_stats(base_resource) :
	...
	def logsdroppedtxminnsbs(self) :
		r"""Total number of log messages dropped by Citrix ADC when NSBQ length is less than TX min NSBs.
		"""
		try :
	...
	def clientconnfail(self) :
		r"""Failures in establishment of a connection between the Citrix ADC and the auditserver tool (the Citrix ADC's custom logging tool).
		"""
		try :
	...
	def auditportallocfail(self) :
		r"""Number of times the Citrix ADC failed to allocate a port when sending a syslog message to the syslog server(s).
		"""
		try :
	...
	def systcpconnfail(self) :
		r"""Failures in establishment of a connection between the Citrix ADC and the syslog server.
		"""
		try :
	...
	def auditmemallocfail(self) :
		r"""Failures in allocation of Access Gateway context structure. When an Access Gateway session is established, the Citrix ADC creates an internal context structure , which identifies the user and the IP address from which the user has logged in.
		"""
		try :
	...
	def logsdropped(self) :
		r"""Total number of log messages dropped by Citrix ADC after max hold limit is reached.
		"""
		try :
