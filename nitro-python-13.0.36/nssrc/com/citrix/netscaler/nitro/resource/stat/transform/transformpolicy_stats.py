...
class transformpolicy_stats(base_resource) :
	r""" Statistics for URL Transformation policy resource.
	"""
	def __init__(self) :
		self._name = None
	...
	def _get_nitro_response(self, service, response) :
		...
		"""
		try :
			result = service.payload_formatter.string_to_resource(transformpolicy_response, response, self.__class__.__name__.replace('_stats',''))
	...
	def  get(cls, service, name="", option_="") :
		r""" Use this API to fetch the statistics of all transformpolicy_stats resources that are configured on netscaler.
		 set statbindings=True in options to retrieve bindings.
		"""
		try :
			obj = transformpolicy_stats()
			if not name :
...
class transformpolicy_response(base_response) :
	def __init__(self, length=1) :
		...
		self.severity = ""
		self.sessionid = ""
		self.transformpolicy = [transformpolicy_stats() for _ in range(length)]
