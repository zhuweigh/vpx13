...
class streamidentifier_stats(base_resource) :
	r""" Statistics for identifier resource.
	"""
	def __init__(self) :
		self._name = None
		self._pattern = None
		...
		self._streamobjpktcredits = 0
		self._streamobjpktspersecond = 0
		self._streamobjdroppedconns = 0

	@property
	def name(self) :
		r"""Name of the stream identifier.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		r"""Name of the stream identifier.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	@property
	def pattern(self) :
		r"""Values on which grouping is performed are displayed in the output as row titles. If grouping is performed on two or more fields, their values are separated by a question mark in the row title.
	...
	def streamobjconn(self) :
		r"""Current connections on the stream session.
		"""
		try :
	...
	@property
	def streamobjdroppedconns(self) :
		r"""Total connections dropped.
		"""
		try :
			return self._streamobjdroppedconns
		except Exception as e:
			raise e

	@property
	def streamobjbreachcnt(self) :
		r"""Total Breaching Transactions in configured interval.
		"""
		try :
	...
	def streamobjresptime(self) :
		r"""Average response time of the stream session.
		"""
		try :
	...
	@property
	def streamobjpktcredits(self) :
		r"""Calculated Packet credits on current connection.
		"""
		try :
			return self._streamobjpktcredits
		except Exception as e:
			raise e

	@property
	def streamobjpktspersecond(self) :
		r"""Total packets recieved in configured interval.
		"""
		try :
			return self._streamobjpktspersecond
		except Exception as e:
			raise e

	@property
	def streamobjreq(self) :
		r"""Total number of Stream Requests recieved.
		"""
		try :
	...
	def  get(cls, service, name="", option_="") :
		r""" Use this API to fetch the statistics of all streamidentifier_stats resources that are configured on netscaler.
