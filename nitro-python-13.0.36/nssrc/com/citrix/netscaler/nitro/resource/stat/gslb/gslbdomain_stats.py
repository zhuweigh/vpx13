...
class gslbdomain_stats(base_resource) :
	r""" Statistics for GSLB domain resource.
	"""
	def __init__(self) :
		self._name = None
	...
	@property
	def name(self) :
		r"""Name of the GSLB domain for which to display statistics. If you do not specify a name, statistics are shown for all configured GSLB
	...
	def dnsrecordtype(self) :
		r""".<br/>Possible values = A, AAAA, CNAME, NAPTR.
		"""
		try :
	...
	def dnsqueriesrate(self) :
		r"""Rate (/s) counter for dnstotalqueries.
		"""
		try :
	...
	def  get(cls, service, name="", option_="") :
		r""" Use this API to fetch the statistics of all gslbdomain_stats resources that are configured on netscaler.
